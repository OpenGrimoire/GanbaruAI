...
<section class="flex flex-col gap-4">
  <div class="flex flex-wrap items-start justify-between gap-3">
    <div><h2 class="text-[0.866667rem] font-semibold text-foreground">{t("settings.chat.workspaces.heading")}</h2><p class="mt-1 max-w-2xl text-xs text-muted-foreground">{t("settings.chat.workspaces.description")}</p></div>
  ...
  {#if creationMode}
    <div class="grid gap-3 rounded-lg border border-border bg-card p-4 sm:grid-cols-2">
      <label class="chat-inline-field"><span>{t("settings.chat.workspaces.name")}</span><input bind:value={workspaceName} /></label>
      {#if creationMode === "project"}<label class="chat-inline-field"><span>{t("settings.chat.workspaces.project")}</span><select bind:value={projectId}><option value={null}>{t("settings.chat.workspaces.chooseProject")}</option>{#each projects.projects.filter((project) => project.status === "active") as project}<option value={project.id}>{project.name}</option>{/each}</select></label>{/if}
    ...
    <div class="flex flex-col gap-5">
      {#each workspaceGroups as group (group.id)}
        <section class="flex flex-col gap-3">
          <h3 class="text-xs font-semibold text-muted-foreground">{group.label}</h3>
      {#each group.workspaces as workspace (workspace.workspace.id)}
        ...
        <article class="rounded-lg border border-border bg-card/50 p-4 {workspace.workspace.archivedAt ? 'opacity-70' : ''}">
          <div class="flex flex-wrap items-start justify-between gap-3">
            <div class="min-w-0 flex-1">
              {#if renameId === workspace.workspace.id}<div class="flex max-w-sm gap-2"><input class="h-8 min-w-0 flex-1 rounded border border-border bg-background px-2 text-sm" bind:value={renameValue} onkeydown={(event) => { if (event.key === "Enter") void commitRename(workspace); if (event.key === "Escape") renameId = null; }} /><button type="button" class="chat-settings-button" onclick={() => void commitRename(workspace)}>{t("chat.save")}</button></div>{:else}<h3 class="truncate text-sm font-semibold text-foreground">{workspace.workspace.displayName}</h3>{/if}
              <div class="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground"><span>{project?.name ?? t("chat.standalone")}</span><span>{bindingLabel(workspace)}</span>{#if workspace.canonicalPath}<span class="max-w-md truncate" title={workspace.canonicalPath}>{workspace.canonicalPath}</span>{/if}</div>
              {#if workspace.workspace.repositoryIdentity}<div class="mt-1 truncate text-[0.666667rem] text-muted-foreground">{t("settings.chat.workspaces.repository")}: {workspace.workspace.repositoryIdentity}</div>{/if}
              {#if workspace.lastVerifiedAt}<div class="mt-1 text-[0.666667rem] text-muted-foreground">{t("settings.chat.workspaces.lastVerified", formatDateTime(localization.locale, new Date(workspace.lastVerifiedAt), { dateStyle: "medium", timeStyle: "short" }))}</div>{/if}
            </div>
            <label class="chat-inline-field min-w-44"><span>{t("settings.chat.workspaces.providerPreference")}</span><select value={preference} onchange={(event) => void perform(workspace.workspace.id, () => chat.setWorkspaceProviderPreference(workspace.workspace.id, event.currentTarget.value || null))}><option value="">{t("settings.chat.workspaces.noPreference")}</option>{#each providers as provider}<option value={provider.configuration.instanceId}>{provider.configuration.label}</option>{/each}</select></label>
          </div>
          <p class="mt-2 text-[0.666667rem] text-muted-foreground">{t("settings.chat.workspaces.pathDeviceOnly")}</p>
          <div class="mt-3 flex flex-wrap gap-1.5 border-t border-border/60 pt-3">
            ...
            {#if workspace.bindingStatus === "available"}<button type="button" class="chat-settings-button" onclick={() => void perform(workspace.workspace.id, () => chat.openWorkspaceFolder(workspace.workspace.id))}><FolderOpen size={13} />{t("settings.chat.workspaces.openFolder")}</button><button type="button" class="chat-settings-button" onclick={() => void perform(workspace.workspace.id, () => chat.removeWorkspaceBinding(workspace.workspace.id))}><Unlink size={13} />{t("settings.chat.workspaces.unbound")}</button>{/if}
            <button type="button" class="chat-settings-button" onclick={() => { renameId = workspace.workspace.id; renameValue = workspace.workspace.displayName; }}>{t("settings.chat.workspaces.rename")}</button>
...
<style>
  ...
  :global(.chat-inline-field input), :global(.chat-inline-field select) { min-height: 2rem; border-radius: 0.375rem; border: 1px solid var(--border); background: var(--background); padding: 0.25rem 0.5rem; color: var(--foreground); }
</style>
