  ...
  <div class="message-body">
    ...
    {#if showReplyStrip && message.replyThread}
      <button type="button" class="reply-strip" data-reply-thread-id={message.replyThread.id} onclick={(event) => onOpenThread(event.currentTarget)}>
        ...
        <span class="last-activity">{formatDateTime(localization.locale, Date.parse(message.replyThread.lastActivityAt), { dateStyle: "medium", timeStyle: "short" })}</span>
...
<style>
  ...
  .last-activity { min-width:0; overflow:hidden; color:var(--muted-foreground); text-overflow:ellipsis; white-space:nowrap; }
