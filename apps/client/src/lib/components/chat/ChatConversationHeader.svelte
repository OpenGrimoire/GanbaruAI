...
<header class="chat-conversation-header @container">
  ...
  <div class="header-actions">
    {#if workspace?.bindingStatus === "available"}<button type="button" class="chat-header-action" title={t("chat.openFolder")} onclick={() => run(() => chat.openWorkspaceFolder(workspace.workspace.id))}><FolderOpen size={14} /><span class="hidden @min-[620px]:inline">{t("chat.header.open")}</span></button>{/if}
    {#if workspace?.currentBranch}<span class="chat-branch" title={t("chat.header.branch", workspace.currentBranch)}><GitBranch size={13} /><span>{workspace.currentBranch}</span></span>{/if}
