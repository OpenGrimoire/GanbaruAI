...
<div class="absolute inset-0 z-30 flex flex-col bg-background" role="dialog" aria-modal="true" aria-label={t("chat.archiveBrowser.title")}>
  ...
  <div class="grid gap-2 p-3"><label class="relative"><span class="sr-only">{t("chat.archiveBrowser.search")}</span><Search size={14} class="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground" /><input class="h-9 w-full rounded-md border border-border bg-background pl-8 pr-2 text-sm" type="search" bind:value={search} placeholder={t("chat.archiveBrowser.search")} /></label><select class="h-9 rounded-md border border-border bg-background px-2 text-xs" bind:value={projectId}><option value="">{t("chat.archiveBrowser.allProjects")}</option>{#each projects.projects as project}<option value={project.id}>{project.name}</option>{/each}</select></div>
