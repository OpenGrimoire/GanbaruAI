<script lang="ts">
  ...
  let remoteSearchResults = $state<ChatThreadShellRead[]>([]);
  let remoteSearchQuery = $state("");
  let remoteSearchLoading = $state(false);
  let searchTimer: number | null = null;
  let searchRequest = 0;
  const threads = $derived.by(() => {
    const local = filterArchivedThreads(chat.archivedThreads, search, projectId);
    const normalizedQuery = search.trim();
    if (!normalizedQuery || remoteSearchQuery !== normalizedQuery) return local;
    const merged = new Map(local.map((thread) => [thread.id, thread]));
    for (const thread of remoteSearchResults) {
      if (!projectId || thread.projectId === projectId) merged.set(thread.id, thread);
    }
  ...
  onMount(() => {
    void chat.ensureArchivedThreads().catch((cause: unknown) => {
  ...
  $effect(() => {
    ...
    searchTimer = window.setTimeout(() => {
      ...
      void chatApi.searchChatThreadTitles(query, true, 100)
        .then((threads) => {
          if (request !== searchRequest) return;
          remoteSearchResults = threads;
        ...
        .finally(() => {
          if (request === searchRequest) remoteSearchLoading = false;
        });
    }, 150);
  });
...
<div class="absolute inset-0 z-30 flex flex-col bg-background" role="dialog" aria-modal="true" aria-label={t("chat.archiveBrowser.title")}>
  ...
  <div class="min-h-0 flex-1 overflow-y-auto px-3 pb-3">
    {#if (chat.archivedThreadsLoading || remoteSearchLoading) && threads.length === 0}<p class="py-8 text-center text-sm text-muted-foreground">{t("common.loading")}</p>{:else if threads.length === 0}<p class="py-8 text-center text-sm text-muted-foreground">{t("chat.archiveBrowser.empty")}</p>{/if}
