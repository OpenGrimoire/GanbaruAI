    ...
    <div class="min-h-0 overflow-y-auto p-4">
      ...
      {:else}
        ...
        {#if mode !== "duplicate"}
          ...
          <fieldset class="mt-3"><legend class="text-[0.7rem] font-medium">{t("music.builder.intendedUses")}</legend><div class="mt-1.5 flex flex-wrap gap-1.5">{#each useOptions as use}<button type="button" aria-pressed={intendedUses.includes(use)} onclick={() => toggleUse(use)} class="rounded-full border border-border/70 px-2.5 py-1 text-[0.68rem] aria-pressed:border-primary aria-pressed:bg-primary/10">{useLabel(use)}</button>{/each}</div></fieldset>
