...
<div class="flex shrink-0 flex-wrap items-center gap-1.5 border-t border-border/70 bg-card px-2 py-2 shadow-[0_-8px_24px_color-mix(in_srgb,var(--background)_45%,transparent)]">
  <span class="mr-1 text-[0.68rem] font-semibold">{t("music.builder.selectedCount", selectedCount)}</span>
  ...
  <button type="button" onclick={onClear} class="ml-auto grid h-7 w-7 place-items-center rounded-md bg-secondary" aria-label={t("music.builder.clearSelection")}><X size={13} /></button>
</div>

<style>
  .bulk-button { display: inline-flex; height: 1.75rem; align-items: center; gap: 0.3rem; border-radius: 0.375rem; background: var(--secondary); padding-inline: 0.5rem; color: var(--secondary-foreground); font-size: 0.63rem; font-weight: 500; }
</style>
