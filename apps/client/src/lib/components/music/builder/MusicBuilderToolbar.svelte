...
<div class="flex h-12 shrink-0 items-center justify-between gap-3 px-3">
  <div class="flex min-w-0 shrink-0 items-center gap-1.5">
    {#if showPanelButton}<button type="button" onclick={onOpenPanel} class="grid h-8 w-8 place-items-center rounded-lg text-muted-foreground hover:bg-secondary hover:text-foreground" aria-label={t("music.builder.openContextPanel")} title={t("music.builder.openContextPanel")}><PanelLeft size={14} /></button>{/if}
