...
<div class="absolute inset-0 z-60 grid place-items-center bg-background/65 p-3 backdrop-blur-sm">
  <div role="dialog" aria-modal="true" aria-labelledby="music-bulk-classification-title" class="flex max-h-full w-full max-w-md flex-col overflow-hidden rounded-xl border border-border/70 bg-card shadow-2xl">
    ...
    <footer class="flex justify-end gap-2 border-t border-border/60 p-3"><button type="button" onclick={onClose} class="h-8 rounded-md bg-secondary px-3 text-xs font-medium">{t("music.builder.cancel")}</button><button type="button" onclick={() => { void save(); }} disabled={controller.saving} class="h-8 rounded-md bg-primary px-3 text-xs font-semibold text-primary-foreground disabled:opacity-40">{controller.saving ? t("music.builder.saving") : t("music.builder.applyChanges")}</button></footer>
  </div>
</div>
