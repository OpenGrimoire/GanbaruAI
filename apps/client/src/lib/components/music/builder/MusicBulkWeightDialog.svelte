...
<div class="absolute inset-0 z-60 grid place-items-center bg-background/65 p-3 backdrop-blur-sm">
  <div use:containMusicDialogFocus={{ onEscape: onClose, escapeDisabled: controller.saving }} role="dialog" aria-modal="true" aria-labelledby="music-bulk-weight-title" tabindex="-1" class="flex max-h-full w-full max-w-sm flex-col overflow-hidden rounded-xl border border-border/70 bg-card shadow-2xl">
    <div class="min-h-0 overflow-y-auto p-4"><div class="flex items-center gap-3"><div class="grid h-9 w-9 place-items-center rounded-lg bg-primary/10 text-primary"><Gauge size={17} /></div><div><h2 id="music-bulk-weight-title" class="text-sm font-semibold">{t("music.builder.bulkWeightTitle")}</h2><p class="mt-0.5 text-[0.68rem] text-muted-foreground">{t("music.builder.bulkWeightDescription", controller.itemIds.length)}</p></div></div>
