...
<div bind:this={root} class="relative" onfocusout={handleFocusOut}>
  ...
  {#if open}
    <div bind:this={popover} tabindex="-1" class="soundscape-popover absolute bottom-full right-0 z-40 mb-2 max-h-[calc(100vh-1rem)] w-[min(17rem,calc(100vw-1rem))] overflow-y-auto rounded-xl border border-border bg-popover p-2.5 text-popover-foreground shadow-md" role="dialog" aria-label={t("music.soundscape.controls")}>
      ...
      <div class="mt-2 grid grid-cols-3 gap-1.5">
        {#each soundscape.definitions.filter((entry) => entry.sourceKind === "generated-noise") as definition (definition.id)}
          <button type="button" aria-pressed={active?.id === definition.id} class={cn("truncate rounded-md bg-secondary px-2 py-1.5 text-[0.68rem] hover:bg-accent", active?.id === definition.id && "ring-1 ring-primary")} onclick={() => { void soundscape.play(definition.id); }}>{definition.generatedKind ? t(`music.soundscape.generatedName.${definition.generatedKind}`) : definition.name}</button>
        {/each}
      </div>
