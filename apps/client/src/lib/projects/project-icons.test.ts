...
describe("project icon values", () => {
  it("parses legacy Lucide slugs", () => {
    expect(parseProjectIcon("folder")).toEqual({ kind: "lucide", slug: "folder", color: "default" });
    expect(serializeProjectIcon(parseProjectIcon("folder"))).toBe("lucide:folder");
  });
  ...
  it("parses explicit empty, emoji, Lucide, custom emoji, and asset values", () => {
    ...
    expect(parseProjectIcon("lucide:rocket:blue")).toEqual({ kind: "lucide", slug: "rocket", color: "blue" });
  ...
  it("serializes values using stable prefixes", () => {
    ...
    expect(serializeProjectIcon({ kind: "lucide", slug: "folder", color: "green" })).toBe("lucide:folder:green");
  ...
  it("falls back to the folder icon for malformed values", () => {
    ...
    expect(parseProjectIcon("lucide:rocket:99")).toEqual({ kind: "lucide", slug: "rocket", color: "default" });
    expect(parseProjectIcon("emoji:")).toEqual({ kind: "lucide", slug: "folder", color: "default" });
  });

  it("maps legacy icon colors to event palette slots", () => {
    expect(projectIconColorToEventColor("default")).toBeUndefined();
    expect(projectIconColorToEventColor("blue")).toBe(18);
