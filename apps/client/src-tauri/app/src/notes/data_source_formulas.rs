...
pub(in crate::notes) fn canonical_formula_config(value: Option<&Value>) -> Result<Value, String> {
    let object = value
...
pub(in crate::notes) fn ensure_formula_schema(properties: &Value) -> Result<(), String> {
    formula_properties_from_schema(properties).map(|_| ())
}

pub(in crate::notes) fn hydrate_formulas(
...
pub(in crate::notes) fn formula_property_value(property_id: &str, payload: Value) -> Value {
    json!({
        "id": property_id,
        "type": "formula",
        "formula": payload
...
pub(in crate::notes) fn formula_plain_text(payload: &Value) -> String {
...
pub(in crate::notes) fn formula_number(payload: &Value) -> Option<f64> {
    match formula_value_from_formula_payload(payload) {
...
pub(in crate::notes) fn formula_checked(payload: &Value) -> Option<bool> {
    match formula_value_from_formula_payload(payload) {
