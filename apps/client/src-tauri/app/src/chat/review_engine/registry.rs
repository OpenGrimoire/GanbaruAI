...
pub(super) const DEFAULT_PATCH_PAGE_BYTES: usize = 1024 * 1024;
...
pub(super) struct ReviewSnapshot {
    ...
    pub(super) source: ReviewDiffSource,
    pub(super) source_label: String,
    pub(super) before_oid: Option<String>,
    pub(super) after_oid: Option<String>,
    pub(super) context_lines: u32,
    pub(super) ignore_whitespace: bool,
    pub(super) files: Vec<ReviewFileInternal>,
    pub(super) provider_patches: HashMap<String, String>,
    pub(super) patch_cache: tokio::sync::Mutex<HashMap<String, Arc<PatchIndex>>>,
    pub(super) object_store: Option<Arc<ReviewObjectStore>>,
    pub(super) created_at: Instant,
}

#[derive(Clone)]
pub(super) struct ReviewFileInternal {
    pub(super) read: ReviewFileRead,
}
...
pub(super) async fn snapshot_read(
...
pub(super) fn stale_snapshot_read(snapshot: &ReviewSnapshot) -> ChatReviewOpenRead {
    ChatReviewOpenRead {
        snapshot_id: snapshot.snapshot_id.clone(),
        review_revision: snapshot.review_revision.clone(),
        source: snapshot.source.clone(),
...
impl ChatReviewRegistry {
    ...
    pub(super) fn insert(&self, snapshot: Arc<ReviewSnapshot>) -> ChatResult<()> {
    ...
    pub(super) fn complete(
        &self,
        id: String,
        operation: CompletedReviewOperation,
    ) -> ChatResult<()> {
        let mut store = self.snapshots.lock().map_err(|_| registry_error())?;
...
pub(super) fn completed_operation_key(database_identity: &str, operation_id: &str) -> String {
    format!(
        "{:x}",
...
pub(super) fn file_id(revision: &str, path: &str, previous: Option<&str>) -> String {
    let mut hasher = Sha256::new();
    hasher.update(revision);
    hasher.update([0]);
    hasher.update(path);
...
pub(super) fn action_request_fingerprint(
