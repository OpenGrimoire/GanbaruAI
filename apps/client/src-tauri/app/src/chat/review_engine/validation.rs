...
pub(super) const MAX_CONTEXT_LINES: u32 = 100;
const MAX_FILE_SELECTION: usize = 100;

pub(super) fn source_requires_thread(source: &ReviewDiffSource) -> bool {
    matches!(
        source,
...
pub(super) fn thread_required() -> ChatError {
...
pub(super) fn validate_open_request(request: &OpenChatReviewRequest) -> ChatResult<()> {
...
pub(super) fn validate_patch_request(request: &ReadChatReviewPatchesRequest) -> ChatResult<()> {
...
pub(super) fn validate_action_request(request: &ApplyChatReviewActionRequest) -> ChatResult<()> {
    validate_opaque_id(&request.snapshot_id, "snapshotId")?;
...
pub(super) fn validate_reference(value: &str, field: &str) -> ChatResult<()> {
    if value.is_empty()
        || value.len() > 1_024
        || value.starts_with('-')
        || value.chars().any(char::is_whitespace)
...
pub(super) fn validate_path(path: &str) -> ChatResult<()> {
    if path.is_empty()
        || path.len() > 4_096
        || path.starts_with('/')
        || path.contains('\0')
