...
pub(in crate::notes) async fn import_folder<R: Runtime>(
    app: &AppHandle<R>,
    db_url: &str,
    pool: &SqlitePool,
    request: NoteNotionExportImportRequest,
) -> Result<NoteNotionExportImportDto, String> {
    import_folder_inner(Some(app), Some(db_url), pool, request).await
}
...
pub(super) async fn import_folder_without_file_copy(
    ...
    import_folder_inner::<tauri::Wry>(None, None, pool, request).await
}

async fn import_folder_inner<R: Runtime>(
    ...
    let runtime = RuntimeImportContext { app, db_url };
...
struct RuntimeImportContext<'a, R: Runtime> {
...
async fn parse_document_entry<R: Runtime>(
    runtime: &RuntimeImportContext<'_, R>,
...
async fn csv_data_source_from_export<R: Runtime>(
    runtime: &RuntimeImportContext<'_, R>,
    prepared: &PreparedExportImport,
    entry: &ExportEntry,
    rows: Vec<Vec<String>>,
...
async fn rewrite_markdown_references<R: Runtime>(
    runtime: &RuntimeImportContext<'_, R>,
    prepared: &PreparedExportImport,
    entry: &ExportEntry,
    target_map: &HashMap<String, ExportTarget>,
...
async fn rewrite_html_references<R: Runtime>(
    runtime: &RuntimeImportContext<'_, R>,
    prepared: &PreparedExportImport,
    entry: &ExportEntry,
    target_map: &HashMap<String, ExportTarget>,
...
async fn rewrite_media_reference<R: Runtime>(
    runtime: &RuntimeImportContext<'_, R>,
    prepared: &PreparedExportImport,
    entry: &ExportEntry,
    reference: &str,
