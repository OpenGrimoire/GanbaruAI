...
#[derive(Clone)]
pub(in crate::notes) struct RelationConfig {
    pub(in crate::notes) target_data_source_id: String,
...
pub(in crate::notes) fn canonical_relation_payload(value: &Value) -> Result<Value, String> {
...
pub(in crate::notes) fn relation_property_value(property_id: &str, payload: Value) -> Value {
    json!({
        "id": property_id,
        "type": "relation",
        "relation": payload,
...
pub(in crate::notes) fn relation_config_from_schema(
    schema: &Value,
) -> Result<Option<RelationConfig>, String> {
...
pub(in crate::notes) async fn ensure_relation_schema_targets_tx(
...
pub(in crate::notes) async fn rebuild_data_source_relation_links_tx(
    tx: &mut Transaction<'_, Sqlite>,
    data_source_id: &str,
...
pub(in crate::notes) async fn replace_row_relation_links_tx(
    tx: &mut Transaction<'_, Sqlite>,
    source_data_source_id: &str,
    source_page_id: &str,
    schema_properties: &Value,
...
pub(in crate::notes) async fn replace_relation_property_links_tx(
    tx: &mut Transaction<'_, Sqlite>,
    source_data_source_id: &str,
    source_page_id: &str,
...
pub(in crate::notes) async fn hydrate_relation_titles_tx(
    tx: &mut Transaction<'_, Sqlite>,
