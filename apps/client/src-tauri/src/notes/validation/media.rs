...
pub(in crate::notes) fn validate_media_payload(
    block_type: &str,
    payload: &Value,
) -> Result<(), String> {
...
pub(in crate::notes) fn validate_local_media_metadata(
...
pub(in crate::notes) fn validate_managed_file_asset_path(
    path: &str,
    field: &str,
) -> Result<(), String> {
...
pub(in crate::notes) fn local_media_content_type_matches_block(
    block_type: &str,
    content_type: &str,
) -> bool {
    match block_type {
...
pub(in crate::notes) fn validate_media_url(
    block_type: &str,
    url: &str,
    allow_empty: bool,
    field: &str,
...
pub(in crate::notes) fn media_url_matches_block_type(block_type: &str, url: &reqwest::Url) -> bool {
    match block_type {
        "file" => true,
...
pub(in crate::notes) fn path_has_extension(url: &reqwest::Url, extensions: &[&str]) -> bool {
    let path = url.path().to_ascii_lowercase();
    extensions.iter().any(|extension| path.ends_with(extension))
}

pub(in crate::notes) fn is_youtube_video_url(url: &reqwest::Url) -> bool {
...
pub(in crate::notes) fn validate_bookmark_payload(payload: &Value) -> Result<(), String> {
    match payload.get("caption") {
        Some(Value::Array(caption)) => {
            for item in caption {
                validate_rich_text_item(item)?;
...
pub(in crate::notes) fn is_bookmark_url_string(url: &str) -> bool {
    !url.chars().any(char::is_control)
}

pub(in crate::notes) fn validate_embed_payload(payload: &Value) -> Result<(), String> {
    match payload.get("url") {
...
pub(in crate::notes) fn validate_link_preview_payload(payload: &Value) -> Result<(), String> {
    match payload.get("url") {
        Some(Value::String(url)) if !url.chars().any(char::is_control) => Ok(()),
        Some(Value::String(_)) => {
