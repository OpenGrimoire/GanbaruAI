...
pub(in crate::notes) async fn get_data_source_timeline_view(
    ...
    let dto = load_timeline_view_tx(&mut tx, data_source_id).await?;
    tx.commit()
        .await
...
pub(in crate::notes) async fn update_data_source_timeline_view(
    ...
    ensure_timeline_view_row_tx(&mut tx, &data_source, &schema).await?;
    sqlx::query(
        "UPDATE notes_database_views
         ...
         WHERE data_source_id = ? AND type = 'timeline'",
    )
...
async fn load_timeline_view_tx(
    ...
    let view = ensure_timeline_view_row_tx(tx, &data_source, &schema).await?;
...
async fn ensure_timeline_view_row_tx(
    ...
    if let Some(view) = load_timeline_view_row_tx(tx, &data_source.id).await? {
        return Ok(view);
    }
    ...
    .fetch_one(&mut **tx)
    .await
    .map_err(|e| format!("prepare notes timeline view order: {e}"))?;
    ...
    load_timeline_view_row_tx(tx, &data_source.id)
        .await?
...
async fn load_timeline_view_row_tx(
    ...
    .await
    .map_err(|e| format!("load notes timeline view: {e}"))
}
