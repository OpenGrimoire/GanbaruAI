...
pub(in crate::notes) fn canonical_rollup_config(value: Option<&Value>) -> Result<Value, String> {
    let object = value
...
pub(in crate::notes) async fn ensure_rollup_schema_targets_tx(
...
pub(in crate::notes) async fn invalidate_rollup_cache_for_data_source_tx(
    tx: &mut Transaction<'_, Sqlite>,
    data_source_id: &str,
) -> Result<(), String> {
    sqlx::query(
...
pub(in crate::notes) async fn invalidate_rollup_cache_for_page_tx(
    tx: &mut Transaction<'_, Sqlite>,
    page_id: &str,
) -> Result<(), String> {
...
pub(in crate::notes) async fn hydrate_rollups_tx(
    tx: &mut Transaction<'_, Sqlite>,
    source_data_source_id: &str,
...
pub(in crate::notes) fn rollup_property_value(property_id: &str, payload: Value) -> Value {
    json!({
        "id": property_id,
        "type": "rollup",
        "rollup": payload
...
pub(in crate::notes) fn rollup_plain_text(payload: &Value) -> String {
...
pub(in crate::notes) fn rollup_number(payload: &Value) -> Option<f64> {
