...
pub(in crate::notes) fn canonical_button_config(value: Option<&Value>) -> Result<Value, String> {
...
pub(in crate::notes) fn ensure_button_schema(properties: &Value) -> Result<(), String> {
...
pub(in crate::notes) fn hydrate_buttons(
...
pub(in crate::notes) fn button_property_value(property_id: &str, label: &str) -> Value {
    json!({
        "id": property_id,
        "type": "button",
        "button": {
...
pub(in crate::notes) async fn click_data_source_button(
    pool: &SqlitePool,
    data_source_id: &str,
    page_id: &str,
