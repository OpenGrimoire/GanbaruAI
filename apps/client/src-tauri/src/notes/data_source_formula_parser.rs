...
pub(in crate::notes) const MAX_FORMULA_EXPRESSION_CHARS: usize = 2_000;
const MAX_AST_NODES: usize = 256;
const MAX_FUNCTION_ARGS: usize = 16;

#[derive(Clone, Debug)]
pub(in crate::notes) enum FormulaExpr {
    Number(f64),
    String(String),
    Boolean(bool),
    Unary {
...
pub(in crate::notes) enum UnaryOp {
    Negate,
    Not,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(in crate::notes) enum BinaryOp {
    Add,
    Subtract,
    Multiply,
    Divide,
...
pub(in crate::notes) fn parse_formula_expression(expression: &str) -> Result<FormulaExpr, String> {
...
pub(in crate::notes) fn formula_dependencies(
    expr: &FormulaExpr,
