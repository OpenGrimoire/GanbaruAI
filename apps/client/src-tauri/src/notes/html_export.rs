...
use super::models::{
    ...
    NoteHtmlArchiveSaveDto, NoteHtmlExportAssetDto, NoteHtmlExportDiagnosticDto, NoteHtmlExportDto,
    NoteHtmlExportFileDto, NoteHtmlExportRequest, NotePageRow,
};
...
pub(super) async fn pick_and_write_archive<R: Runtime>(
    ...
    let mut archive = build_archive(pool, request).await?;
    html_export_archive::pick_and_write_archive(app, &mut archive)
}
...
pub(super) struct HtmlArchive {
    pub(super) dto: NoteHtmlExportDto,
