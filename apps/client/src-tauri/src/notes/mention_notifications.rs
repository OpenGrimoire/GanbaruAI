...
pub(in crate::notes) async fn list_pending(
    pool: &SqlitePool,
...
pub(in crate::notes) async fn mark_delivered(
    pool: &SqlitePool,
...
pub(in crate::notes) async fn refresh_all(pool: &SqlitePool) -> Result<i64, String> {
    let mut tx = pool
        .begin()
        .await
...
pub(in crate::notes) async fn sync_block_tx(
...
pub(in crate::notes) async fn sync_comment_tx(
    tx: &mut Transaction<'_, Sqlite>,
    comment_id: &str,
    page_id: &str,
    block_id: Option<&str>,
...
pub(in crate::notes) async fn clear_source_tx(
    tx: &mut Transaction<'_, Sqlite>,
    source_type: &str,
    source_id: &str,
) -> Result<(), String> {
