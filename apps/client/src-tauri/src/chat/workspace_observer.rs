...
struct WorkspaceChangeAccumulator {
    scope: ObserverScope,
    generation: u64,
    git_roots: Vec<PathBuf>,
...
impl WorkspaceChangeAccumulator {
    fn new(scope: ObserverScope, generation: u64, git_roots: Vec<PathBuf>) -> Self {
        Self {
            scope,
            generation,
            git_roots,
    ...
    fn push(&mut self, message: ObserverMessage) {
        match message {
    ...
    fn push_relative_path(&mut self, path: String) {
    ...
    fn finish(self) -> Option<ChatWorkspaceChangeBatch> {
...
fn safe_workspace_relative_path(root: &Path, path: &Path) -> Option<String> {
    ...
    if value.len() > 4_096
        ...
        || super::workspace_files::observer_excluded(&value)
...
fn discover_git_metadata_roots(root: &Path) -> Vec<PathBuf> {
...
fn git_watch_paths(git_roots: &[PathBuf]) -> Vec<(PathBuf, RecursiveMode)> {
...
fn is_git_metadata_path(path: &Path, root: &Path, git_roots: &[PathBuf]) -> bool {
...
fn git_metadata_relative_is_relevant(value: &str) -> bool {
    value.is_empty()
        || matches!(
            value,
...
mod tests {
    ...
    fn accumulator_filters_generated_symlink_and_git_paths() {
        ...
        fs::create_dir_all(directory.0.join(".git/refs/heads"))
