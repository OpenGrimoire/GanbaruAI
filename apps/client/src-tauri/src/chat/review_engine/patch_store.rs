...
pub(super) struct ParsedHunk {
    pub(super) read: ReviewHunkRead,
    pub(super) text: String,
}

pub(super) struct ParsedPatch {
    pub(super) preamble: String,
    pub(super) hunks: Vec<ParsedHunk>,
}

pub(super) enum PatchIndex {
    Memory(ParsedPatch),
    Spool(SpoolPatchIndex),
}

#[derive(Clone)]
pub(super) struct SpoolPatchIndex {
...
pub(super) struct ReviewObjectStore {
    root: PathBuf,
    pub(super) objects: PathBuf,
    pub(super) index: PathBuf,
    pub(super) alternate_objects: PathBuf,
}
...
pub(super) async fn read_patch_page(
...
pub(super) fn unavailable_patch(file_id: &str) -> ReviewFilePatchRead {
    ReviewFilePatchRead {
        file_id: file_id.to_string(),
        patch: None,
        hunks: Vec::new(),
...
impl ReviewObjectStore {
    pub(super) async fn new(repository_root: &Path) -> ChatResult<Self> {
    ...
    pub(super) fn verify_size(&self) -> ChatResult<()> {
        let mut total = 0_u64;
