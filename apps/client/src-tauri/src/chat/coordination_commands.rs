...
pub async fn chat_schedule_message(
    ...
    let pool = chat_pool(app, db_url).await?;
    let channel = super::channel_commands::read_channel(&pool, &request.message.channel_id).await?;
    if channel.archived_at.is_some() {
        return Err(ChatError::new(
            ChatErrorCode::Conflict,
...
pub async fn chat_list_scheduled_messages(
    ...
    let pool = chat_pool(app, db_url).await?;
    super::channel_commands::read_channel(&pool, &channel_id).await?;
...
fn validate_scheduled_for(scheduled_for: &UtcTimestamp, now: &UtcTimestamp) -> ChatResult<()> {
...
async fn claim_scheduled_message_for_immediate_send(
    pool: &SqlitePool,
    scheduled_message_id: &ChatScheduledMessageId,
    now: &UtcTimestamp,
    claim_token: &str,
