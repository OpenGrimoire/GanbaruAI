...
|---|---:|---:|---:|---:|---:|---|
| Projects | 1 | 6 | 0 | 13,142 | 355 | Yes |
| Notes | 1 | 7 | 0 | 323 | 355 | Yes |

Rust library tests count typed initial-load commands, executed SQLite statements, and serialized payload bytes against the fixture. Component tests keep the backend promises unresolved while asserting that the useful shell is present. They also prove that empty Notes performs no history or auxiliary page request and that paragraph content becomes ready without waiting for auxiliary metadata. The production bundle contract emits module metadata during the Vite build and enforces the route-chunk module ceilings above. Calendar, Projects, Notes, Chat, the Music panel, Quick notes, the global Settings modal, its top-level sections, the Project settings panel, and the Notes project settings panel constructors are resident. Chat accepts a bounded resident shell so first navigation does not wait for a route chunk. The organizational feed, reply thread with direct provider-rendered teammate responses, and participant mention behavior add eight net resident source modules after replacing the obsolete channel timeline cache. The Chat ownership refactor adds 11 focused resident source modules without changing the heavy lazy boundaries. The shared typed message-action target adds one small resident source module while replacing duplicated channel and execution action behavior. The three-level Chat breadcrumb adds two focused resident presentation modules for project-to-channel navigation. The Chat identity and settings redesign adds five net focused modules for identity previews, teammate drafts, provider configuration, sensitive-value redaction, and shared switches after removing the obsolete model and provider cards. Because Projects, Notes, and Chat share the resident App shell, all three measured route closures contain 355 source modules and use a 355-module ceiling. The shared Pomodoro scheduler, progress ring, and control menu add three focused resident modules to the desktop shell. Android keeps its shell ceiling unchanged by loading the scheduler after vault initialization and the control menu on first interaction. Provider discovery, working-folder reconciliation, archived history, Review renderer, and syntax grammars remain deferred or demand-driven. Quick notes accepts a deliberate 17-module shell increase so its panel and editor open without a deferred component import and its initial collection read can begin at shell mount. The primary Projects views, the Projects navigator, the Notes home, the Notes navigators, and the paragraph editor are resident in their route closures. Projects workspace metadata begins loading at vault activation, followed by lightweight Notes workspace and cross-project navigation metadata, so first navigation can reuse the same single-flight requests while task windows, note bodies, and optional data remain demand-driven. Notes maintenance and transfer surfaces, database views, advanced block families, Settings detail workflows, diagnostics, and the icon catalog remain lazy. Only the selected primary surface and any explicitly opened panel mount, so residency does not activate hidden component effects. `pnpm -w run validate` runs all three contract layers.

The unrecorded `dense-chat-v1` fixture seeds project working folders, 80 organizational channels, 8,000 root messages, 800 reply threads, 2,400 replies, and 100 hidden provider execution sessions. Chat prewarms lightweight active-channel navigation, reads bounded organizational pages, searches message and reply content through SQLite FTS, and keeps organizational and execution pages in separate serialized byte budgets. Provider sessions are not linked to channels or presented as channel history. The fixture version remains unchanged because no comparable benchmark row has been recorded for this shape.

Global Settings opens synchronously and switches among its top-level sections without constructor-loading states. The production bundle contract requires the modal and every top-level section in the resident closure. Notes transfer workflows, transfer dialogs, and the Doomscrolling limit editor remain separate lazy detail chunks because they are uncommon, heavier workflows rather than navigation surfaces.
