...
Harness v1 is the current recorded baseline after the 2026-05-12 reset. It runs scenarios against isolated benchmark databases, records only the measurement type each scenario needs, then shows readable summary tables and copies normalized Markdown for [performance records](results.md). Startup and idle memory run both dense calendar datasets: `dense-v1-r1y-s1-d1` and `dense-v1-r10y-s1-d1`. Practical user-window scenarios run only `dense-v1-r1y-s1-d1`.

Harness and dataset versions are tied to recorded rows, not local iteration. Do not bump `HARNESS_VERSION`, `DENSE_DATASET_VERSION`, or dense detail profile names until the current version has at least one run in [performance records](results.md). While tuning an unrecorded benchmark shape, edit the current version in place.
...
The harness uses device-local `app_config_dir/benchmark.sqlite` for the whole run. The user's real database and Ganbaru AI folder are not opened during benchmark passes.
...
`apps/client/src/lib/benchmark/registry.ts` is the boundary. `BENCHMARK_SCENARIOS` contains metadata only. `loadScenarioById()` owns dynamic imports for executable scenario modules.
...
|---|---|---|
| `startup-boot` | `apps/client/src/lib/benchmark/scenarios/startup-boot.ts` | Repeated launch to usable calendar paint |
| `idle-memory` | `apps/client/src/lib/benchmark/scenarios/idle-memory.ts` | Anchored week idle RAM |
| `calendar-nav` | `apps/client/src/lib/benchmark/scenarios/calendar-nav.ts` | Physical right-arrow hold week-view RAM |
| `calendar-panel-latency` | `apps/client/src/lib/benchmark/scenarios/calendar-panel-latency.ts` | Calendar panel open latency from existing-event and create actions |
| `calendar-import-ops` | `apps/client/src/lib/benchmark/scenarios/calendar-import-ops.ts` | Rust-backed calendar bulk import latency |
...
- A `Copy markdown` action that copies plain canonical Markdown for [performance records](results.md), not HTML, raw scenario diagnostics, or every collected metric.
...
The placeholder exists because the app cannot know which sequence number the canonical record will use. When adding copied rows to [performance records](results.md), replace `-ID` with the next zero-padded suffix for that date, such as `-01` or `-02`. Do not leave `-ID` in canonical rows.
...
The copied Markdown intentionally avoids slash-packed memory cells, repeated global metadata, empty stat columns, redundant dataset levels, diagnostic counters, and measurement-level notes. Run metadata is the only generated table with a `Notes` column. Fixed scenario details belong in this harness spec, while [performance records](results.md) contain rows and minimal column interpretation. A future comparable detail should become a dedicated typed column instead of a generic note.
...
- `apps/client/src/lib/benchmark/types.ts`: scenario contract, pass result types, suite state, versions, sampling constants.
- `apps/client/src/lib/benchmark/dense.ts`: deterministic dense calendar dataset generation.
- `apps/client/src/lib/benchmark/registry.ts`: lightweight scenario metadata and dynamic scenario loaders.
- `apps/client/src/lib/benchmark/runner.ts`: pass orchestration, persisted state, restart wiring.
- `apps/client/src/lib/benchmark/sampler.ts`: memory sampling and boot timing capture.
- `apps/client/src/lib/benchmark/output.ts`: benchmark result markdown formatter.
...
- `apps/client/src/lib/components/calendar/nav-handle.svelte.ts`: headless calendar driver for benchmark scenarios.
- `apps/client/src/lib/api/db.ts`: benchmark DB selection through `vaultMode`.
- `apps/client/src-tauri/app/src/desktop_runtime.rs`: benchmark state commands, database prepare and teardown, restart, startup elapsed time, and memory report.
