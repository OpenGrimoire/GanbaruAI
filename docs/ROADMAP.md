...
- An initial one-WebView shell with phone bottom navigation, a larger-window rail, lazy Calendar, Projects, Notes, Settings, and Quick notes surfaces, mobile Calendar day mode, Projects list mode, Notes adaptations, Pomodoro, a shared capability-aware Settings system, VisualViewport tracking, a bounded native four-edge inset bridge, native system-bar theme contrast, nested root-yielding Back handling, and localized mobile copy
...
Remaining Storage Access Framework export, backup, restore, attachment, and media adapters, notification, alarm, deep-link and Media3 adapters, encrypted user-controlled backup, complete adaptive backup UX, predictive Back and gesture-navigation validation, emulator validation, durable signing-key provisioning, signed release acceptance, and distribution are not implemented yet.
