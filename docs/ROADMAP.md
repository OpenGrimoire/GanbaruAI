...
- A build-time mobile frontend entry and adapter graph that reuses shared Music and Doomscrolling configuration through Android adapters while excluding the desktop App shell, desktop Chat execution, Rodio player, native desktop media controls, desktop Doomscrolling process APIs, benchmark surfaces, and working-folder panels from Android production assets
- Permission-free Android document selection for preflighted managed image assets and Notes database CSV files up to 512 KiB, including browser and Rust MIME, dimension, and pixel-count checks, plus a native bounded directory-tree importer for existing Ganbaru AI vaults and streaming portable backup and transactional restore; remaining larger attachment transfers stay behind native streaming content-URI adapters
...
- Android selected-app Doomscrolling with targeted launchable-app discovery, Usage Access accounting, separately disclosed Accessibility Service blocking, native Pomodoro phase delivery while the Activity is absent, daily and weekly package limits, protected system destinations, bounded device-local journaling, canonical SQLite import, and full read-only Browser and Desktop configuration tabs

Remaining Storage Access Framework attachment adapters, notification and deep-link adapters, native local-playlist queue handoff, encrypted scheduled backup, predictive Back and gesture-navigation validation, emulator validation, durable signing-key provisioning, signed release acceptance, and distribution are not implemented yet.
...
- Responsible Android Doomscrolling: opt-in Usage Access, targeted package visibility, no broad installed-app access, no overlay or content inspection, and a separately disclosed user-enabled Accessibility Service limited to deterministic selected-package Home redirection
