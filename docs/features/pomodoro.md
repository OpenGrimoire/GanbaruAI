...
The user can pause manually, the system can auto-pause for idle, and desktop can register a suspend pause when the computer sleeps. All three are handled with the same data structure (a row in `pomodoro_pauses`), but their UX differs (see `features/pomodoro-idle-detection.md`). Android backgrounding, WebView throttling, screen sleep, and Activity process eviction do not create suspend pauses because the native schedule remains authoritative. Paused time never counts as focus progress or break credit. If the event deadline becomes the limiting factor while manually paused, the title bar and tray countdown continue to show the usable focus opportunity left in the event, while the calendar rail keeps the paused interval empty.
...
On Android, Calendar reconciliation persists a bounded schedule of future Pomodoro-enabled occurrences and exact activation alarms. The schedule, current phase projection, activation and boundary receivers, and ongoing notification service are owned by one lightweight private `:guardian` process rather than the Tauri Activity process. The Activity publishes and reads projections through a non-exported provider, and it never opens the guardian's runtime preferences directly. At an event boundary, Android can start the ongoing notification and `specialUse` foreground service even when the Activity and WebView are absent. Opening the app then reconciles that temporary native activation with the canonical SQLite run. Closing the Activity or removing its task leaves the active service, notification, and session running under standard Android behavior. Its main title is the calendar event name when available, with the current focus or break phase as the fallback. It shows no redundant secondary phase label while running, but shows Paused as the secondary label when manually paused. A system countdown and horizontal progress indicator show phase progress. A native alarm advances its persisted phase projection when the Activity or WebView is absent, while cold recovery validates and applies those elapsed boundaries to SQLite. Manual pause holds the current phase instead of advancing it in the background. Phase changes post a separate alert and never force the Activity to the foreground. Android 14 and newer let users dismiss most ongoing notifications with an individual swipe, so canonical timer continuity cannot depend on notification visibility. OEM task cleaners can suppress foreground-service restart outside standard Android behavior. Android Focus settings therefore expose manufacturer autostart and unrestricted-battery setup where applicable.
