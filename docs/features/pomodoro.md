...
On Android, an active session also owns one ongoing system notification. Its main title is the calendar event name when available, with the current focus or break phase as the fallback. It shows no redundant secondary phase label while running, but shows Paused as the secondary label when manually paused. A system countdown and horizontal progress indicator show phase progress. A native alarm advances its persisted phase projection when the Activity or WebView is absent, while cold recovery validates and applies those elapsed boundaries to SQLite. Manual pause holds the current phase instead of advancing it in the background. Phase changes post a separate alert and never force the Activity to the foreground. Android 14 and newer let users dismiss most ongoing notifications with an individual swipe, so canonical timer continuity cannot depend on notification visibility.
