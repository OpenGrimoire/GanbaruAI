...
- Settings > Doomscrolling is split into Limits, Browser, Mobile apps, and Desktop apps. Limits are usage budgets that apply whenever Ganbaru AI is running. Browser supports enable or disable, blocking during focus, blocking during short breaks, blocking during long breaks, pausing while the focus session is paused, Blacklist mode, Whitelist mode, blocked websites, blocked categories, custom category stacks, exceptions, and allowed websites. Mobile apps is a work-in-progress placeholder. Desktop app limit entries are selected from the installed-app picker rather than typed manually.
