...
Usage limits are budget rules, not Pomodoro phase rules. They apply whenever Ganbaru AI is running. Daily budgets reset at local midnight, and weekly budgets reset with the Monday-based local week.
...
- Android mobile usage counts only explicitly selected packages while the screen is interactive and unlocked. It stores package identifier, display label, elapsed seconds, local date, and timing. It does not store screen content, text, taps, notifications, or a general app history.
