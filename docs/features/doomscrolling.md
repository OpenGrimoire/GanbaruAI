...
The timer projection alone grants no blocking authority. Android usage awareness requires deliberate Usage Access, and selected-app blocking requires a separately disclosed and user-enabled Accessibility Service. Ganbaru AI explicitly declares `isAccessibilityTool=false` and `canRetrieveWindowContent=false`, subscribes only to window-state package changes, does not request gesture or screenshot capabilities, and does not inspect view trees, screen text, taps, passwords, or notifications. Its automation is deterministic: when a user-selected static trigger matches a user-defined rule, it performs the standard Home action. The onboarding presents Usage Access and Accessibility Service disclosures separately immediately before opening the matching system setting. The Accessibility disclosure explains the observed app identity, local selected-app usage and block records, enforcement purpose, and no-sharing policy before requiring affirmative consent. Domain filtering still requires an explicitly disclosed local VPN, and any device-owner path requires its own enrollment and policy design. The mobile subsystem fails open when either required access or its validated rule snapshot is unavailable, while reporting that protection is inactive the next time the app is visible. Google Play distribution also requires an accurate Accessibility API declaration, prominent in-app disclosure and affirmative consent because Ganbaru AI is not an accessibility tool, plus listing documentation and review under the current [AccessibilityService policy](https://support.google.com/googleplay/android-developer/answer/10964491?hl=en).
