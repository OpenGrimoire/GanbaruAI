...
The local profile represents a human participant. Stable AI roles such as the Ganbaru manager have distinct labeled identities, and provider accounts or models are execution metadata rather than people. The UI must not let an AI role impersonate the local person or a future collaborator.
