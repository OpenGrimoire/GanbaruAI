...
2. **View a built-in**. The detail view uses the same grouped editor surface as user themes, projected from the built-in into a full read-only source and token snapshot. The JSON panel shows that full snapshot with Copy on every platform and Save to file where the platform exposes the scoped file boundary.
3. **Edit a user theme**: rename it, choose Color defaults for Calendar surface, Event palette, and Calendar details, tweak any of the 32 event-palette hexes (including alpha) through an in-house HSV color picker, edit the source colors to shift the shell in lockstep, and click Isolated edit on any driven row to flag it user-pinned and unlock direct hex input (Link back unflips it and re-derives). Faded event variants blend the slot RGB toward the calendar canvas while preserving that slot's alpha. Destructive, Confirm, and Warning expose only background/text source pairs; the old per-action and per-status signal rows are internal aliases of those pairs. The sun/moon icon in the header is a clickable decorative tag (day/night use) that the user can flip independently of canvas luminance; the flip is persisted on the theme but does not affect the runtime `.dark` class. The same JSON panel is editable; pressing Apply changes validates through `replaceThemeDraft`, updates the in-memory session in place with its id locked, and leaves final persistence to Save and apply.
4. **Reset a single color** back to its clone-time state. Source rows enable their reset icon when the source hex differs from the seed snapshot. Driven token rows enable their reset icon only when their isolated flag differs from the seed snapshot, or when the row is already isolated and its own hex differs from the seed. Linked driven rows do not reset independently just because their derived value changed after a source edit; they reset through the source that drives them. Clicking an enabled row reset restores the source value or the driven row's seed isolation state. If the seed state is linked, the row re-links to the current derivation instead of writing a stale clone-time hex.
5. **Reset the theme** in one click via the "Reset all to seed" button in the footer. Restores every source, every app and calendar token (value and isolated flag), the event palette, the calendar default selection, the decorative icon tag, and `blendCanvas` from their seed mirrors in one in-memory pass. Like every editor mutator, the result lives in `$state` until Save and apply commits it. When the theme is already at its seed state, the button is disabled with the same muted opacity and not-allowed cursor used by the Appearance reset controls.
...
7. **Share** a theme by exporting it from the theme row or detail view. Copy JSON writes to the clipboard. Export and Save to file disable their initiating control and replace its download icon with a progress spinner until the platform operation settles, preventing duplicate writes. A successful write shows a dismissible success notification for three seconds, while a failed write remains visible longer for recovery. Desktop and iOS Save to file use a backend-owned native save dialog. Android creates an `application/json` entry in the public Downloads collection through MediaStore because a save destination adds little value for small shareable theme files and vendor document providers can be unreliable. Its success notification names the final Downloads file. Android import still uses the system document picker because the user must choose an existing document. The webview never receives a selected path or content URI. Rust and the Android native adapter both restrict transfers to 1 MiB, and Android rejects malformed UTF-8 before returning text. No broad storage permission or generic frontend filesystem capability is granted. Import accepts pasted JSON or a picked `.json` document and then applies the normal theme validator. Imported themes are rejected when their incoming ID collides with a built-in or an existing user theme, so files keep their explicit identity instead of being silently renamed.
...
Per-row reset compares the live editor state against the seed entry on the same `UserTheme`, but linked driven rows are treated as children of their source. A source row surfaces an enabled reset when its source hex differs from the seed. A driven token row surfaces an enabled reset when the live isolated flag differs from the seed isolated flag, or when the row is currently isolated and its own hex differs from the seed. A linked row whose value only drifted because App canvas, Ink, Primary action, or Color defaults changed keeps a disabled reset control with muted opacity, because the correct reset target is the parent source. Clicking an enabled reset restores the source value, restores a seed-isolated token to its seed hex with `isolated=1`, or re-links a token whose seed state was linked so it follows the current derivation immediately. "Reset all to seed" runs the equivalent restore across every seed bucket (sources, app tokens, calendar tokens, palette, blend canvas, and icon tag) in one in-memory pass. Both flow back to SQLite only when the editor commits.
...
Chat syntax colors are also runtime-only implementation details. Files and Review derive the same foreground, comment, keyword, constant, string, parameter, function, expression, punctuation, link, invalid, inserted, and deleted roles from the resolved app signals against `--cal-bg`. Each role walks perceptual lightness until it reaches 4.5:1 contrast, then becomes a local `--chat-syntax-*` variable. CodeMirror maps Lezer token classes to those variables. Pierre uses a CSS-variable Shiki theme that references the same values, so a theme change repaints highlighted files and diffs without retokenizing them. These roles are not members of `APP_TOKEN_KEYS` or `CALENDAR_TOKEN_KEYS`, are not persisted or exported, and do not require a theme migration.
...
The test suite asserts: every derivable body-text pair meets AA 4.5:1; status/destructive/confirm pairs meet AA-large 3:1 when their text source is configured for that target; muted captions park in `[3.0, 5.0]` (the upper bound accommodates gamut clamping on near-white canvases); event-panel divider sits at or above 1.4:1 against the panel; calendar gridline sits at or above 1.4:1 against cal-bg; calendar time label sits at or above 3:1 against cal-bg; `sidebar <= canvas <= card <= popover <= accent` OKLab-L ordering holds on every canvas with headroom; feeding BASE.dark's sources reproduces BASE.dark exactly on source-driven and hardcoded tokens except the accepted-status foreground, and within 2 rgb-units on every shift-derived or walk-fraction token.

The derivable calendar tokens are `--cal-bg`, `--cal-gridline`, `--cal-time-label`, and `--cal-timeline-rail`. Calendar header is the app token `--cal-header-bg`; it follows App canvas by default and can still be isolated from the App canvas section. `--cal-current-time` (red "now" line), `--cal-timeline-break` (black break marker), and `--cal-timeline-focus` (green pomodoro marker) are no longer derived from sources: those accents carry semantic meaning that does not reduce cleanly to the source palette, so the snapshot stores the BASE CSS hex on clone and users can isolate any of them to repaint it. Date picker today chips use the primary action colors instead of calendar-specific theme rows.
...
- **Calendar surface**: UI description: "Calendar background, gridlines, and time labels"; a sourceless group covering the calendar background and its driven tokens. Sub-rows expose `--cal-bg` (isolate to pin a specific grid color), `--cal-gridline`, and `--cal-time-label`. Isolating `--cal-bg` protects the grid color from source-edit cascades and rebakes until the user applies another Color default or links the row back.
- **Event palette**: UI description: "32 color slots, each one has a faded variant for optional past-event dimming, blended toward Calendar background"; appears directly below Calendar surface because event colors are previewed against the calendar background and are usually tuned together. Applying Color defaults replaces the full slot set, but each slot can still be edited afterward.
- **Calendar details**: UI description: "Rail states and accents on the calendar grid"; a sourceless group that collects the calendar details a user can reasonably tune: now line, empty rail, break marker, and focus marker. The empty rail picker appears before the break marker picker because both describe states of the same pomodoro rail.
...
**Peer-style rows** (used when a source-driven group has exactly one sub-row): the row uses the same bold header typography (`text-[13px] font-semibold`) so the driven token reads as a peer of its source rather than a subordinate option. The right side uses the normal token editor controls. Linked rows show a read-only `ColorField`, disabled reset control, and **Isolated edit** action. Isolated rows show an editable `ColorField`, enabled row reset when applicable, and **Link back**.
...
Rows in groups without a local source header still use the same Linked and Isolated states. Calendar surface and Calendar details follow Color defaults; Event panel follows App canvas. Linked rows remain read-only until the user chooses **Isolated edit**, and their reset control stays disabled unless the row's own isolated state differs from the seed. The color value uses the normal UI font, is vertically centered in the color field, and can be edited in the picker as HEX, RGB, or HSV from a compact format row once isolated.
...
- **Calendar details** (4, sourceless): now line, empty rail, break marker, and focus marker. Break and focus use BASE CSS values on clone (`#000000` for break in both built-ins, `#39965c` for light focus, and `#2a8049` for dark focus); users can still isolate any of these rows to repaint them.
...
Runtime-only implementation colors include `--event-panel-edge`, `--event-panel-shadow`, `--event-panel-divider`, `--event-panel-input-text`, `--event-panel-placeholder`, `--form-indicator`, `--app-scrollbar-thumb`, `--app-scrollbar-thumb-hover`, `--cal-scrollbar-thumb`, and `--cal-scrollbar-thumb-hover`. They are resolved for painting, but they are not exported, imported, stored in `theme_tokens`, or shown in the visual editor. The app scrollbar colors are derived from the active foreground and apply automatically to native vertical and horizontal scrollbars. Native scrollbar tracks, corners, and buttons stay transparent. Rounded thumbs keep fixed geometry and change only color on hover or interaction, with horizontal thumbs slightly thicker than vertical thumbs for easier targeting. Hovering a horizontal scrollbar shows the localized Shift plus Scroll down or up shortcut directly above its rail. New scrollable UI inherits this treatment by default. Surfaces with dedicated overlay scrollbars continue to hide the native scrollbar and may derive their thumb colors from their own surface. Chat derives its light-surface Full access text by shifting the warning signal toward the destructive signal and then toward the active foreground, producing a darker orange, while dark mode uses the warning signal directly. The event color picker outline, description editor edit tint, and all-day drag-preview border are component-derived styles rather than theme tokens.
...
- **Interface scale:** whole-interface zoom is separate from font scale. Desktop uses Tauri's native WebView zoom levels. Android persists a curated 75% to 150% scale and applies CSS content zoom because Tauri does not support its native WebView zoom command on Android or iOS. The mobile shell inversely compensates its viewport dimensions so scaling never exposes an unpainted edge. At larger scales, compact navigation can omit redundant visible labels while retaining accessible names. The mobile range supports both denser layouts and accessibility enlargement without exposing the extreme desktop-only levels that make a phone interface impractical.
