...
2. **View a built-in**. The detail view uses the same grouped editor surface as user themes, projected from the built-in into a full read-only source and token snapshot. The JSON panel shows that full snapshot with Copy and Save buttons.
