...
7. **Share** a theme by exporting it from the theme row or detail view. Copy JSON writes to the clipboard. Desktop and iOS Save to file use a backend-owned native save dialog. Android creates an `application/json` entry in the public Downloads collection through MediaStore because a save destination adds little value for small shareable theme files and vendor document providers can be unreliable. Android import still uses the system document picker because the user must choose an existing document. The webview never receives a selected path or content URI. Rust and the Android native adapter both restrict transfers to 1 MiB, and Android rejects malformed UTF-8 before returning text. No broad storage permission or generic frontend filesystem capability is granted. Import accepts pasted JSON or a picked `.json` document and then applies the normal theme validator. Imported themes are rejected when their incoming ID collides with a built-in or an existing user theme, so files keep their explicit identity instead of being silently renamed.
