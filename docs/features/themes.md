...
7. **Share** a theme by exporting it from the theme row or detail view. Copy JSON writes to the clipboard; Save to file calls a backend-owned native save dialog that starts in the OS Downloads folder when available and only writes `.json`. Import accepts pasted JSON or a `.json` file picked and read by the backend with a 1 MiB cap. Imported themes are rejected when their incoming ID collides with a built-in or an existing user theme, so files keep their explicit identity instead of being silently renamed.
