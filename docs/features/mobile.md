...
- Remaining Storage Access Framework backup, restore, and attachment transfers, notification and alarm boundaries, deep links, contextual runtime permissions, encrypted user-controlled backup, native local-playlist queue handoff, and broader Android media-format acceptance.
...
- Compact width places the small set of primary destinations as icon-only controls in the global top bar. Their accessible names preserve clarity without repeating visible labels or reserving a bottom bar.
