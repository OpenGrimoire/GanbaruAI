...
- Complete mobile Settings and backup UX, restoration for remaining noncanonical drafts and navigation state, physical process-death recovery, root-yielding and predictive Back validation, and remaining touch or narrow-layout adaptations inside shared feature components.
...
Unavailable features are removed from primary navigation. A contextual explanation is shown only where a shared record refers to a desktop action. Permanent disabled desktop controls create noise and should not be used as the normal mobile presentation.
