...
- An Android capability grants the `main` WebView Back-listener registration and removal, scoped HTTPS URL opening, the official notification commands required to check and request runtime permission, the narrow app-owned commands required to reconcile Calendar deliveries and consume a validated notification tap, and explicit mobile Doomscrolling commands for access status, settings recovery, launchable-app selection, rule publication, journal reads, acknowledgements, and notification actions. Calendar channel creation and scheduling remain behind the app-owned Android notification adapter. Cross-window Calendar, Quick notes, and theme synchronization resolves to a build-time no-op transport because Android owns one WebView. Android therefore receives no general core event authority. It also receives no unencrypted HTTP, email, telephone, file revealing, or path opening authority. The desktop capability retains its separate desktop window, event, and updater authority.
- The mobile command surface exposes app-private SQLite behavior for Calendar, Projects, Notes, provider-free Chat communication, Pomodoro, Quick notes, Themes, Vault, managed assets, Doomscrolling usage and block history, and the shared Music library. Android Music uses a narrow native Media3 and document-tree adapter. Android Doomscrolling uses a separate native selected-package adapter and never compiles the desktop process-closing implementation. Chat reuses the shared channel, message, reply-thread, search, draft, and scheduling contracts while excluding provider processes and workspace execution. The mobile composition also excludes desktop notification and overlay windows, desktop audio and tray integrations, soundscapes, benchmarks, working-folder Markdown, and generic native path pickers.
- First use reuses the desktop setup content and visual hierarchy while describing the actual mobile choice. Android does not offer an alternate live-vault location, so the primary action is the iconless `Start from zero` rather than a default-folder choice. A concise `Data location` row still shows the resolved Android path for transparency. The action creates the canonical vault under Tauri's app data directory as `Ganbaru AI` or `Ganbaru AI Dev`. Restore explicitly selects the portable `.ganbaru-backup` file created by Android, while import explicitly selects a complete existing Ganbaru AI folder, such as one copied from desktop. Import opens Android's native directory-tree picker, streams the bounded folder into a private staging directory, validates it with the shared vault boundary, and atomically activates the imported copy.
- Android Data settings create a consistent portable vault backup in shared Downloads and restore that format through the system document picker. Backup snapshots SQLite with `VACUUM INTO`, streams files into a bounded archive without passing their bytes through JavaScript, and excludes live WAL and SHM files. Restore extracts into app-private staging with path, depth, entry-count, and expanded-size limits, validates the vault marker and SQLite integrity, blocks new database connections, closes existing pools, and atomically replaces the active vault with rollback on activation failure. First use can restore the same backup after reinstalling.
- After vault setup, Android shows one compact Focus access screen using the same full-height visual hierarchy. It shows only notification and exact-alarm access that the current Android version reports missing, plus Background autostart when a verified manufacturer settings adapter is available, battery-saver review, Usage Access when missing, and App blocking when the Accessibility Service is disabled. Each row keeps its name and description on the left with one Review action on the right. Usage Access and App blocking each show a separate localized disclosure and require affirmative agreement immediately before opening the corresponding Android setting. Supported access refreshes from Android after returning from Settings. Manufacturer autostart controls expose no supported status API, so the screen records that the user returned from reviewing the control without displaying a false enabled state. Continue unlocks after five seconds and persists completion locally. Focus and Doomscrolling settings retain the same direct recovery actions.
...
- Android Doomscrolling persists a bounded, versioned rule snapshot under the active vault identity. The Pomodoro notification scheduler publishes each native phase transition to the blocking adapter, so selected-app rules continue while the Activity and WebView are closed. The enabled Accessibility Service receives only window-state package changes, rejects Ganbaru AI, Home, Settings, the dialer, system UIDs, unknown packages, stale projections, and packages outside the selected rule set, then performs the standard Home action for a matching rule. Usage Access provides selected-package foreground time for daily and weekly limits and service-start reconciliation. A bounded SQLite journal splits usage at local midnight, compacts old pending rows, keeps fourteen days of native totals, and imports current-vault usage and block events into canonical SQLite when the app next opens.
...
- The Android manifest keeps each declared access tied to an implemented adapter. Doomscrolling adds only `PACKAGE_USAGE_STATS`, targeted launchable-activity visibility, and the user-enabled Accessibility Service declaration. It does not add broad package visibility, overlay authority, screen capture, gesture injection, or view-content retrieval. Android backup and device transfer exclude the unencrypted app-private vault and all other application storage until an intentional encrypted backup design exists. The capture FileProvider exposes only the app-scoped `Pictures/` directory.
...
- Remaining Storage Access Framework attachment transfers, Notes notification boundaries, deep links, encrypted scheduled backup, native local-playlist queue handoff, and broader Android media-format acceptance.
- Restoration for remaining noncanonical drafts and navigation state, broader physical process-death recovery, root-yielding and predictive Back validation, and remaining touch or narrow-layout adaptations inside shared feature components.
- Android device and emulator validation, Play policy work, and a production Android release.
- Mobile sync, remote Chat execution, sleep alarm, and Android website filtering.
...
Unavailable features are removed from primary navigation, but mobile Settings preserve the desktop category structure so preferences remain predictable across devices. Keyboard Shortcuts is the only omitted category because it is wholly desktop-specific. Each Android section contains shared settings, a real platform equivalent, or a direct route to the existing feature-owned settings surface. Unsupported individual controls are omitted without removing their category or replacing them with an implementation-status card. Doomscrolling is the temporary exception while its Android enforcement is developed.

### Settings architecture and section matrix

Settings use one shared category registry, one controller, and shared section components where the underlying behavior is genuinely portable. Focus exposes the same preference set on desktop and Android, followed by compact Android background-recovery links that use the same settings pattern. These links remain available after the one-time Focus access screen and do not imply that Android can inspect manufacturer-specific controls. Vite resolves the section renderer, detail loader, profile picker, WebView zoom adapter, and Music platform boundaries at build time. Android packages the provider-free Chat workspace but not local provider, terminal, Git, local review-panel, preview, or working-folder execution graphs. It also excludes the updater, general native path-picker, tray, and desktop window-event graphs. The scoped mobile document adapter selects bounded Calendar and theme documents and creates their public Downloads exports without exposing a generic filesystem API to Svelte. The mobile presentation uses a category list followed by a full-screen section. Android Back returns from a section to the category list before closing Settings. Desktop keeps its modal navigation, lazy detail panels, keyboard handling, and draft protection.
...
| Calendars | Shared calendar records, event counts, deletion, bounded ICS import through Android's document picker, and ICS export to Downloads. ZIP bundle import remains desktop-only. |
| Projects | The general category is intentionally empty until global Project preferences exist. Project workflow, defaults, statuses, priorities, fields, and applicable working-folder controls remain in each project's own settings panel. |
| Notes | Shared default-open and version-history retention settings. File transfers and native notification preferences remain hidden until their Android adapters can apply them. |
| Chat | Shared portable behavior settings for restoring the last thread, showing provider reasoning summaries, and automatically folding settled work. Local providers, terminals, diagnostics, and execution controls remain desktop-only. |
...
| Music | Shared Pause music when Focus is paused preference, applied through the Android Media3-backed player with the same automation ownership rules as desktop. |
...
| Data | Shows the active vault identity and its app-private Android storage boundary. Reveal and arbitrary folder switching remain omitted because Android does not expose that private folder as a desktop-style user path. |
| Updates | Shows the installed build and opens the signed release history. The distribution source owns Android installation and update delivery, so the desktop self-updater is not packaged. |
...
| Import, export, and backup | Shared typed transfer plans | Native tree import plus streaming portable backup and transactional restore; remaining transfers use Storage Access Framework content URIs | Native paths and folder dialogs |
...
- Manual backup writes a timestamped `.ganbaru-backup` archive to shared Downloads. It contains the portable vault only, uses a consistent SQLite snapshot, remains after uninstall, and is deliberately user-initiated because it is not encrypted.
- Restore selects that archive through Android's system document UI, streams it into bounded app-private staging, validates archive paths and resource limits, validates the vault marker and database integrity, then replaces the canonical vault transactionally. It is available during first use and from Data settings.
- Restore requests shared Downloads as the system picker's initial location, while whole-vault import requests the primary shared-storage root. These explicit best-effort hints prevent an earlier Music selection from becoming the accidental starting point. Android or an OEM document provider may ignore a requested initial location.
...
Android Auto Backup includes much app-private data by default. The generated manifest therefore disables backup and supplies both legacy backup rules and Android 12 or newer data-extraction rules. Every app storage domain is excluded from cloud backup and device-to-device transfer. This prevents the unencrypted vault, credentials, caches, and device-local state from entering an implicit platform backup. The explicit portable backup is different: the user deliberately creates it in shared Downloads and controls whether it is moved elsewhere. Automatic or cloud backup remains unavailable until an encrypted design and its threat model are reviewed. See [Android Auto Backup](https://developer.android.com/identity/data/autobackup) and [Android backup security guidance](https://developer.android.com/privacy-and-security/risks/backup-best-practices).

Uninstalling the app removes the canonical app-private vault. A manual backup already saved to shared Downloads remains available and can be restored from first use after reinstalling. First use shows the resolved private folder path so the one-folder data model stays explicit, while Data settings state the uninstall consequence directly and provide the durable backup action.
...
1. Usage awareness uses `UsageStatsManager` only after the user grants Usage Access in Android Settings. Native samples are limited to explicitly selected package identifiers, display labels, elapsed seconds, local dates, and timing. See [UsageStatsManager](https://developer.android.com/reference/android/app/usage/UsageStatsManager).
2. App selection avoids broad installed-package enumeration. The native picker queries launchable activities through targeted package visibility, filters the app itself and protected system destinations, and never requests `QUERY_ALL_PACKAGES`. See [Android package visibility](https://developer.android.com/training/package-visibility) and the [Google Play package visibility policy](https://support.google.com/googleplay/android-developer/answer/10158779?hl=en).
3. Selected-app blocking uses a user-enabled Accessibility Service because Android exposes no narrower public app-blocking API for an ordinary consumer app. The service declares `canRetrieveWindowContent=false`, receives only window-state events, reads the originating package identifier, and performs only the standard Home action after a static user-defined rule matches. It does not inspect view trees, text, taps, passwords, notifications, gestures, or screenshots. It is not declared as an accessibility tool. Its separate in-app disclosure and consent flow are mandatory, and Play distribution requires an accurate declaration, listing documentation, and review. Deterministic rule-based automation is allowed by the current [Google Play AccessibilityService policy](https://support.google.com/googleplay/android-developer/answer/10964491?hl=en), subject to approval and all other policies.
4. The service remains active independently of the Activity and WebView because Android manages enabled Accessibility Services. Rules, phase projections, usage totals, and pending block events are bounded and device-local. Missing access, malformed or stale projection state, protected packages, and unknown packages fail open. Force stop remains an explicit Android override until the app is opened again.
5. Domain filtering may use an explicitly disclosed local VPN only after battery, privacy, conflict, emergency bypass, and Play policy design. Device-owner controls and unrestricted overlays are not used. See the [Android VPN service guide](https://developer.android.com/develop/connectivity/vpn).
...
- No broad storage, all-files access, unrestricted package visibility, overlay, background location, or unnecessary foreground service is part of the base app. The declared Accessibility Service has no authority until the user enables it after the separate disclosure.
...
| Accessibility Service binding | User affirmatively enables selected-app blocking after its separate disclosure | Rules and limits remain configurable, but Android does not redirect blocked apps |
...
The app does not pre-request unused sensitive access for current or future features. The base app does not request `MANAGE_EXTERNAL_STORAGE`, legacy broad shared-storage access, `QUERY_ALL_PACKAGES`, `SYSTEM_ALERT_WINDOW`, device-owner status, contacts, microphone, camera, or location. Usage Access and Accessibility Service activation enter separate Doomscrolling enable flows with localized disclosures, denial UX, status recovery, and store-policy review. VPN consent remains absent until Android website filtering has its own completed design. A future feature that genuinely needs another permission requires an updated spec, localized disclosure, denial UX, tests, and store-policy review before the permission enters the manifest.
...
The current source foundation completes the local build, CI wiring, protected signing configuration, and physical-shell portions of milestone 1, parts of milestone 2, the native Focus delivery path, and the first selected-app Doomscrolling adapter. The native Android project builds all four supported ABIs into universal APK and AAB artifacts, while pull requests build only ARM64 for bounded feedback. An ARM64 debug APK installs and passes initial Android 10 acceptance for cold start, background process restart, portrait and landscape insets, compact and wide navigation, system-bar theme contrast, root Back, Calendar, Projects, Notes, Quick notes, Pomodoro, and Settings. Android Doomscrolling still requires physical acceptance across access grant, denial, revocation, Activity removal, reboot, low-memory service restart, daily and weekly rollover, protected packages, and OEM task management, plus the Play Accessibility API declaration and review. Creating and protecting the durable release key, validating the first minified signed artifacts, emulators, and the complete release matrix remain milestone 1 work. Milestones 1 through 4 do not depend on sync and can proceed before Phase 9. Milestone 5 depends on Phase 9. Sleep alarm depends on the diary domain and native alarm design. Policy-sensitive features require their own store-policy review before release.
...
- Doomscrolling Usage Access and Accessibility Service grant, denial, revocation, separate disclosure consent, selected-app filtering, Home redirection, protected packages, paused phases, phase boundaries while the Activity is absent, daily and weekly limit rollover, reboot, package replacement, force stop recovery, journal import, and notification deep links.
