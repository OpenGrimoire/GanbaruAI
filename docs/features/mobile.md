...
- Remaining Storage Access Framework backup, restore, and attachment transfers, Pomodoro and Notes notification boundaries, deep links, encrypted user-controlled backup, native local-playlist queue handoff, and broader Android media-format acceptance.
...
Settings use one shared category registry, one controller, and shared section components where the underlying behavior is genuinely portable. Vite resolves the section renderer, detail loader, profile picker, WebView zoom adapter, and Music platform boundaries at build time. Android packages the provider-free Chat workspace but not local provider, terminal, Git, local review-panel, preview, or working-folder execution graphs. It also excludes Doomscrolling, updater, general native path-picker, tray, and desktop window-event graphs. The scoped theme document adapter is the narrow exception for Settings: an app-owned Tauri mobile plugin selects one import document or creates one public download without exposing a generic filesystem API to Svelte. The mobile presentation uses a category list followed by a full-screen section. Android Back returns from a section to the category list before closing Settings. Desktop keeps its modal navigation, lazy detail panels, keyboard handling, and draft protection.
...
- Global session, timer, notes, media, and settings controls follow the desktop utility order after the primary destinations. They expand into their full control surfaces without adding stacked mini-players or another persistent bar.
...
Exact alarms are not a launch-time request. Android documents Calendar as a valid precise-alarm use case, so Ganbaru AI declares `SCHEDULE_EXACT_ALARM` and offers the app-specific Alarms and reminders screen only after a user enables Calendar reminders and exact access is unavailable. Denial keeps an explicitly described `setAndAllowWhileIdle` fallback. Pomodoro still uses the least privileged scheduling that meets measured reliability. A separate sleep-alarm design may reuse exact access only with contextual education and graceful fallback. See [Android alarm guidance](https://developer.android.com/develop/background-work/services/alarms).
...
| --- | --- | --- |
| `POST_NOTIFICATIONS` on API level 33 and newer | Requested after the user enables timer, calendar, or alarm notifications | In-app deadlines remain correct; the relevant surface reports that background alerts are unavailable and links to Android settings |
| `SCHEDULE_EXACT_ALARM` special access | Offered in context when an Android 12 or newer Calendar user enables reminders and exact access is unavailable; future sleep alarms reuse the same truthful boundary | Use `setAndAllowWhileIdle`, show that reminders can arrive late, and retain an action that opens Alarms and reminders access |
| `RECEIVE_BOOT_COMPLETED` scheduling support | Declared by the app-owned Calendar notification adapter for persisted schedule restoration | Domains without persisted native delivery reconcile only when the app next opens and never imply reboot-safe delivery |
