...
- **ProjectSelector:** pick the project linked to the event. The picker shows groups and projects only. It does not expose sections or tasks in the event panel. Selecting a project saves `project_id` and applies the project's default event name, color, duration, Pomodoro preset, focus playlist id, and work environment id when those fields are available. The default event name is applied only when the current event title is empty. Default duration is not applied to active event edits, so switching projects cannot accidentally resize an event that is currently in progress.
