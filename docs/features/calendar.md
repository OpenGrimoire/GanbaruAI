...
**Month view.** A grid of day cells, each summarizing the day's events as compact chips. When all events fit in the available legible rows, each event uses a full-width row. When event count exceeds the available rows, short chips can share a row while longer chips keep the width they need up to the per-chip cap and wrap to the next compact row. Horizontal chip gaps and vertical row gaps match so the compact layout reads evenly. Spare row width first goes to visible labels that still need readable space, then the last event chip expands into whatever remains. Chips with meeting details reserve and render the generic meeting, call, location, or call and location indicators; repeat indicators stay out of month view. The `+N more` indicator is a compact text button and appears only when the cell cannot fit more legible rows, but it does not expand like an event chip. In very narrow cells, it shortens to `+N` while preserving the full label for accessibility. When clicked, it opens a modal for the full day with a vertical one-event-per-row list. Each row shows the event title and time range, opens the event panel without closing the modal, and the list scrolls only when the event count exceeds the modal's practical height. Dismissing the modal also requests any open event panel to close through the normal discard flow. When `+N more` is present, the row may hide one more real event instead of leaving earlier visible chips unnecessarily truncated. The rail is **not** rendered in month view: the cells are too small for a 6px rail to communicate anything meaningful, and the user is in a planning context, not an in-session context.
...
- **Pomodoro config:** optional on timed events. When present, the event participates in the pomodoro system. All-day events cannot have a pomodoro config. See `features/pomodoro.md`.
...
Event notifications use native platform delivery, not sound-only alerts. The notification title is the event title, and the body includes the lead time, time range or all-day label, and location when available. Desktop keeps a foreground lifecycle scheduler that checks the exact next deadline and catches up briefly after resume. Clicking the notification or its Open calendar action switches the main app to Calendar and requests that the OS show and focus the main window. Ubuntu GNOME and other compositors may still block the raise request through focus-stealing prevention. The app asks the OS not to keep desktop Calendar reminders in notification history after the popup disappears, though final persistence behavior depends on the user's desktop notification server.

Android expands the authoritative SQLite events and recurrence rules into a rolling 12-month schedule containing at most the next 256 native deliveries, then reconciles stable notification IDs at startup, after Calendar mutations, and when the Activity resumes. This bound leaves capacity below Android's per-app alarm limit for other Ganbaru AI domains and avoids retaining years of expanded recurrences. The Tauri notification adapter stores scheduled deliveries in app-private preferences, restores them after reboot, and uses `AlarmManager` so delivery does not require the Activity or WebView to remain alive. Android 13 and newer request notification permission only when the user enables an event reminder. Android 12 and newer use exact alarms when the user grants Alarms and reminders access, with an explicit in-panel recovery action and an honest inexact fallback while access is unavailable. Android 10 requires neither runtime notification permission nor exact-alarm special access. Notification taps launch the existing app and navigate to Calendar after either a warm or cold start. The event panel can send a real test reminder and open the app-specific native sound settings when an OEM suppresses audible delivery.

## Interactions

The day, work-cycle, and week views support direct manipulation:

- **Click and drag on empty space** creates a new event. The drag defines the time range, with start and end snapping to the current timeline grid. Releasing opens the edit panel anchored at the new block.
...
- **Active event start edits are locked.** Body drag, top-edge resize, start date, start time, and all-day conversion are disabled because they would change a start that already happened. Other event details remain editable while the event is in progress. The bottom edge and end fields remain editable.
- **Active event panel action** replaces delete or archive with **End event**. The action uses a scissors icon and sets the event end time to now. For a pomodoro event, this lets the pomodoro run close as an event that has already ended. If no other overlapping pomodoro event can take over, the user confirms with the standard "Stop the focus session?" modal. If another overlapping pomodoro event exists, End event uses the same inline red confirmation pattern as archive/delete. A local active event without a pomodoro config can also be ended this way, but it only writes the cut event end and does not create or close pomodoro data. The same local active no-pomodoro event can still enable the Pomodoro section; saving stores the event edits plus the new pomodoro config and starts a fresh session from the current event window. The ended event can be archived afterward from its past-event state.
- **Esc, clicking a non-interactive calendar surface, or pressing Save** closes the edit panel.
...
- **Tap empty timed space** creates the same one-hour event as a desktop click, starting at the half-hour band under the finger. Movement beyond touch slop cancels the tap before creation, so scrolling and swiping do not leave accidental drafts.
...
- **ProjectSelector:** pick the project linked to the event. The picker shows groups and projects only. It does not expose sections or tasks in the event panel. Selecting a project saves `project_id` and applies the project's default event name, color, event duration, Pomodoro preset, focus playlist id, and work environment id when those fields are available. The default event name is applied only when the current event title is empty. A project all-day duration default switches a non-active timed draft to all day. Default duration is not applied to active event edits, so switching projects cannot accidentally resize an event that is currently in progress.
...
- **Date pickers:** start and end date pickers show the selected event range with highlighted endpoints and a muted in-between span. Today stays bold when it is not an endpoint, and past days keep a muted foreground even when they are part of the selected range. Selecting a start date after the current end date moves the end date to the same day. Selecting an end date before the current start date moves the start date to the same day. Other endpoint edits leave the opposite endpoint unchanged.
- **TimePicker:** start and end time. Hidden when all-day is on.

Unsaved changes are tracked. Closing the panel with unsaved edits prompts to save or discard. Confirmation dialogs opened from the panel own their pointer events, so the panel's outside-close listener must not block clicks on dialog buttons. Saving closes the panel and shows `Saving...` as soon as the confirmed commit starts, then updates that toast to a brief `Saved` success state after persistence succeeds. Deleting or archiving also hides the panel and shows `Deleting...` or `Archiving...` as soon as the confirmed commit starts, while the affected blocks use the final display projection until the store refresh completes. That same toast updates to the final undo state after persistence succeeds. Saving on an event that was already part of a saved recurring series uses the scope picker (this, following, all), except when the selected recurring occurrence is active. Active selected recurring occurrences are isolated to `Only this` and do not show the scope picker. Adding repeat to a saved non-recurring event does not show the scope picker because there is no previous series to scope. See `features/calendar-recurrence-editing.md`.

Past read-only event panels keep editing controls locked, but the Meeting section can still expand for inspection. Opening it does not enable edits or emit a change. All-day event panels hide Pomodoro and Music because both depend on a timed event window. Toggling an unsaved event to all-day hides those sections without destroying their local panel state; toggling back to timed restores the in-progress Pomodoro choices, while saving as all-day persists no Pomodoro config.

Project task scheduling can create calendar events outside the edit panel. Those events still use the normal calendar event schema, carry `project_id`, and can be opened later in the event panel for normal editing.
...
Calendar rendering is windowed. The frontend asks Rust for the visible date range instead of keeping the whole event database in memory. Non-recurring rows are bounded by the visible window, while recurring templates are expanded only for that requested range. View switches are atomic: if the target window is not already covered by real loaded data, the current view remains visible until the complete target snapshot is ready.
...
The store keeps only a small bounded cache of render windows. Day, work-cycle, week, and month views can render from an exact cached window or a cached window that fully covers the requested range, but never from a smaller previous window. Day, work-cycle, week, and month views prefetch adjacent windows after a successful foreground load, and the mounted calendar also prefetches same-anchor alternate views so normal view changes can swap from real cached data. Mutations clear the cache so saved edits, imports, deletes, detach, and split operations cannot reuse stale render state.
...
When a pomodoro session is running on an event, the calendar UI restricts certain edits to keep the timer's assumptions valid. Local active events without pomodoro configs use the same End event path for cutting the current block, but they do not touch pomodoro state. They remain editable while active, except for start-time, top-edge drag, body drag, all-day conversion, and repeat-chain structural edits. They can also enable Pomodoro while active; saving that change stores the new pomodoro config and starts a fresh session from the current moment. The restrictions are not arbitrary: each one corresponds to a hazard documented in `data/hazards.md` (hazard 2).

- **Active events end before archive.** A running pomodoro event shows End event instead of delete or archive. Ending sets the event's end time to now and closes the run as event-expired/completed. Ending the only active pomodoro candidate uses the standard "Stop the focus session?" modal; ending one overlapping pomodoro event uses inline red confirmation. A local active event without a pomodoro config can use the same End event action without touching pomodoro tables, and it can enable Pomodoro from the normal editable panel. The user can then archive the resulting past event.
...
- **Recurring delete scopes preserve protected occurrences.** `Only this` archives or deletes only the selected concrete occurrence. From a started selected occurrence, `Following` and `All` preview and archive only started occurrences in the requested scope, leaving future mutable occurrences in the repeat chain. From a future selected occurrence, `All` preserves protected history and removes the mutable future side by capping the series at the last started occurrence, or at the captured edit date when archived exceptions already removed those generated rows. Active occurrences are excluded from repeat-chain archive/delete actions until they are ended. On commit, one semantic plan applies the final visible projection in one step and sends one atomic backend batch. Affected protected occurrences archive first, then any structurally removed future occurrences disappear from the visible series.
...
- **Start-time edits are blocked.** The session already started; segments already exist. Direct manipulation cannot move the active block or resize its top edge, and the panel locks the start date, start time, and all-day toggle. End-time edits remain available.
- **Recurrence edits are scoped carefully.** Editing the active occurrence of a recurring event is forced to `Only this`; the scope picker is hidden so the active run cannot accidentally rewrite its repeat chain. `Following` and `All` edits made from another occurrence leave the active occurrence unchanged. If the active occurrence would otherwise vanish because it is after a `Following` split, Save materializes it unchanged and transfers the active run to that standalone row. See `features/calendar-recurrence.md`.
...
- **Import .ics file.** Calls the Rust command `vault_pick_and_read_ics_import`, which opens a native file picker filtered to `.ics` and `.zip`, starts in the user's Downloads directory when the OS exposes one, validates the selected path in Rust, and reads only after the user chooses a file. Plain `.ics` files are capped at 25 MiB, parsed via `lib/calendar/ics/parser.ts`, and bulk-upserted by `(calendar_id, source_uid)`. A `.ics.zip` (Google Calendar's default export shape, where each calendar is a separate `.ics` inside the bundle) is unpacked in Rust with the `zip` crate using only the deflate feature enabled. The zip path validates entry paths through `enclosed_name` (zip-slip protection), refuses encrypted entries, and caps each entry at 25 MiB / aggregate 250 MiB / 1024 entries to reject decompression bombs. Each `.ics` entry becomes its own imported calendar (one toast aggregating the totals). Re-importing the same file always lands in the same calendar keyed by the source filename, so events deduplicate by UID instead of stacking. Imported calendar UI labels show the `.ics` basename and an `ics` tag; the settings details show the event count plus a localized import date and time. Warnings (lossy fields, unknown TZIDs, malformed events) are shown under the import controls with repeated messages deduped, and are also logged to the console for diagnostics.
...
The complete iCalendar compatibility spec lives in `docs/interop/icalendar/`. It defines the target standards scope, the preservation and normalized projection architecture, fixture strategy, client-specific test guidance, edit merge policy, and performance budget for broad RFC-compatible import and export.

New imports store iCalendar objects and components in relational preservation tables. Projected events, attendees, alarms, and recurrence overrides link back to preserved components so full event loads can expose preservation status, projection warnings, and reconstructed component data for export merging. The visible calendar window still reads only normalized event rows. Export merges linked preserved `VEVENT` and nested `VALARM` components with the app's current supported fields, so unsupported event properties, unsupported parameters, inert URI attachments, imported `DURATION` shape, floating date-time shape, and `RECURRENCE-ID;RANGE=THISANDFUTURE` survive supported edits. Preserved `VTIMEZONE` definitions are emitted before generated timezone stubs. Top-level non-event components are passed through unchanged. Rows without preserved components still export through generated components.

When an import contains duplicate master `VEVENT` components with the same `UID`, Ganbaru AI projects the newest revision by `SEQUENCE`, then by `LAST-MODIFIED`, `DTSTAMP`, or `CREATED` when sequence ties. This handles Google exports that include an older uncapped recurrence plus a newer capped recurrence for the same event identity.
