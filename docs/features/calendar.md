...
- **ProjectSelector:** pick the project linked to the event. The picker shows groups and projects only. It does not expose sections or tasks in the event panel. Selecting a project saves `project_id`; while creating a timed event, it also applies the project's default title, color, duration, Pomodoro preset, focus playlist id, and work environment id when those fields are available.
