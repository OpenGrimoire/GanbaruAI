...
Chat settings live in Ganbaru's existing Settings modal and open on Teammates. A short guided flow is prefilled only after a healthy provider is available and requires review before saving. Provider cards retain health, version, hidden account identity, refresh, edit, enable, disable, and removal. Model controls remain scoped to one provider instance and are reused inside the teammate editor. Working-folder bindings and the project's primary manual workspace folder are edited from Project settings.
...
The execution composer and native send path remain internal execution machinery. Organizational posting atomically commits the human message, structured mentions, reply-thread identity, work assignment, frozen context package, and durable dispatch job before provider launch. Hidden provider runs receive a Rust-owned effective configuration resolved from the teammate policy, channel membership, exact folder grant, provider capabilities, native safety policy, device binding, and Full access trust. Provider launch failure preserves the organizational message and exposes Retry plus Edit teammate.
...
One shared work thread can own several sequential or parallel runs when recovery, specialization, or workspace isolation requires them. Each run remains bound to one project working folder and its own provider continuation. The coordination layer aggregates only semantic state, questions, approvals, result summaries, and review links. Tool groups, provider lifecycle, session handoffs, and full Codex-like workspace details remain in the execution projection.
...
These policies can become explicit context actions for the Ganbaru manager, coding-agent runs, and the future general assistant. They shape a bounded context package instead of creating one unbounded general-agent memory for the complete project. They never bypass participant access, selected workspace, provider, model, interaction mode, budget, or safety confirmation.
