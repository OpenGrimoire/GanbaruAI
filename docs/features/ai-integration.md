...
One shared work thread can own several sequential or parallel runs when recovery, specialization, or workspace isolation requires them. Each run remains bound to one exact execution environment, either an authorized working-folder environment, an existing managed worktree, or private scratch, plus its own compatible provider continuation. The coordination layer aggregates only semantic state, questions, approvals, result summaries, and review links. Tool groups, provider lifecycle, session handoffs, and full Codex-like workspace details remain in the execution projection.
