...
Ganbaru AI's AI features are entirely opt-in. The app is fully functional with no AI configured. When users opt in, AI participates through the communication and work model in [Agent coordination](agent-coordination.md). Stable AI roles, manager plans, context packages, task assignments, reviews, and budgets belong to Ganbaru's coordination layer. Providers supply bounded reasoning or execution sessions beneath that layer and never become the canonical identity of a channel, role, task, or decision.
...
The implemented Chat route shell is resident with the other main tabs and reuses the Calendar internal background token. Its lightweight active-channel projection prewarms after Projects metadata without mounting the hidden Chat UI. The same prewarm starts one background provider-detection pass for the active vault, but navigation, drafts, and the selected channel page never await it. Projects, Notes, and Chat share the selected project. The current rail shows project channels and personal sections, keeps Direct messages visibly planned, and removes working folders and provider threads from primary navigation. A channel links ordered hidden provider sessions and exposes their execution target in the composer. The first slice treats every Send as a coding-agent action. Later participant routing allows human-only messages and stable AI roles without changing channel identity. Merely selecting a channel does not start a provider session.
...
Chat settings live in Ganbaru's existing Settings modal. A vault without a Codex instance checks the installed `codex` command during its first Chat prewarm, including conventional user CLI directories that desktop launchers may omit, and creates a default instance that uses the CLI's existing home and authentication. The advanced setup uses Provider, Identity, and Connection steps for additional accounts, custom homes, other provider families, native executable and folder pickers, field-level validation, operating-system credential references, and an explicit probe before use. Provider cards expose health, version, hidden account identity, last successful probe, refresh, edit, enable, disable, and removal. Model controls remain scoped to one provider instance and preserve hidden, favorite, stale, unavailable, deprecated, and supported custom-model states. Working-folder bindings and provider preferences are edited from Project settings.

The implemented execution timeline projects canonical provider events and bounded SQLite pages into messages, work activity, plans, pending state, and settled-turn summaries. Stable row IDs support measured virtualization, page prepend anchoring, loaded-page eviction, fold and expansion state, follow or anchored reading intent, unread counts, and an optional long-run minimap. Consecutive tools use stable activity groups while live and after settlement. The current live tool owns the outer label, nested rows preserve each tool's detail, and completed groups use localized singular, plural, and mixed-kind summaries. User rows expose durable attachments, workspace mentions, bounded terminal context, timestamps, copy, and checkpoint-aware restore entry points. Assistant rows use bounded Marked parsing followed by DOMPurify sanitization, preserve incomplete streaming Markdown, enhance code blocks with copy and wrap controls, and show effective model, usage, and changed-file metadata. Settled process disclosures own the turn duration and keep it separate from semantic activity summaries. Organizational timelines later add human and AI-role messages plus concise links to these detailed run projections.

The implemented execution composer uses one persistent controller per channel. It saves exact prompt text, explicit provider and model selections, typed model traits, safety and interaction modes, workspace mentions, managed image references, and a recoverable sent snapshot. Sending commits the user message, turn, attachment references, idempotent command receipt, and any new channel-session link in one transaction before provider dispatch. Provider launch failures therefore keep a durable channel execution row and support Retry, Edit into a new draft, or Change provider through a new hidden session. The later organizational composer stores human-only room messages independently, then lets an addressed AI role start a manager action or run without making provider startup the durability boundary for human communication.
...
The future manager does not receive a generic power to spawn arbitrary processes or read the complete vault. It creates a typed run request from an approved task or explicit objective. Rust resolves the effective participant, role, conversation, project, working folder, context package, authority policy, budgets, provider capability, and execution environment before a provider worker can start.

A context package contains versioned references or bounded snapshots of the task, acceptance criteria, dependencies, selected Notes, Calendar constraints, relevant conversation context, prior run summaries, applicable instructions, and resource limits. Package construction applies the same access boundary as direct reads. A participant, destination channel, role, or run with narrower access cannot receive a broader summary produced from restricted data.
...
Every AI read applies the intersection of the requesting participant's authority, destination conversation visibility, AI-role policy, context-package selection, and run grants. The same boundary applies to search, mentions, backlinks, notifications, digests, reports, exports, and summaries so restricted content cannot leak through a derived result.
