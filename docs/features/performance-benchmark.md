...
| `dense-chat-v1` | The isolated benchmark DB seeded with the fixed Chat workload: 4 groups, 20 projects and workspaces, 100 threads, 2,000 turns, 4,000 messages, 4,000 activities, 1,000 plans, 500 attachments, 200 checkpoints, and 10,000 canonical events. |
...
| `chat-workspace` | `lib/benchmark/scenarios/chat-workspace.ts` | Chat activation, recent-thread switching, bounded reads, rail search, streamed paint cadence, idle process CPU, memory, and owned process stop |
