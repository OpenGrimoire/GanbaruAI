...
Notes mentions are canonical rich text objects inside validated block payloads and comments. The editable mention slices support page mention objects with local page ids and local Notes hrefs, plus date mention objects for common date and reminder inputs. Reminder mentions store local `ganbaru_reminder` metadata inside the validated date mention object, but notification delivery remains a future local feature. Mention display text is part of the rich text object for import compatibility, while backlinks and navigation use stable page ids where a page target exists. Future user mentions should resolve through `notes_local_users` for the local author and through explicit local or imported user rows when sync or imports introduce other people. Future local object, inline equation, and range comment anchors should extend the rich text model with explicit validated variants rather than storing untyped JSON.

Notes backlinks are derived from canonical block payloads, child-page pairings, and normalized database relation links. The first implementation scans visible active block rows on visible active source pages for paired `child_page` references, local Notes URL links, and page mention payloads, then adds active database relation facts from `notes_data_source_relation_links`. A future `notes_backlinks` table may cache the same facts for search and graph views, but it must remain rebuildable from canonical page, block, rich text, data source, and row property rows.

Notes comments are canonical SQLite rows, not page payload annotations. `notes_comment_threads` stores the discussion id, target page, optional target block, open or resolved state, and resolution metadata. `notes_comments` stores individual rich text comments, the local author id, a display-name snapshot derived from `notes_local_users`, attachments metadata, timestamps, and soft-delete state. Editing the local display name updates existing local comment display-name snapshots so the UI does not leave stale `You` labels behind. `notes_comment_thread_anchors` stores optional inline text-range anchors for block threads with offsets, selected text, and nearby prefix and suffix context. Page comments use a page parent, block comments use a block parent, and visible comment reads hide block threads when the target block is trashed. Inline anchor highlights are derived from the current block text at read time, first by exact offsets and then by context-assisted selected-text search when nearby edits moved the text. Unread state and future sync metadata should extend these tables instead of embedding comments into block payload JSON.
