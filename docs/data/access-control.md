...
Teammates is the primary complete access editor, but it keeps the same dimensions and visual grammar as every other Settings area. The resident view uses the established master-detail layout, with a compact identity directory and a larger editor for the selected teammate. Identity and execution remain directly editable in that detail column. The Channel access section contains one configure action that opens a focused modal matching the Access profiles manager. It does not introduce a wider administration surface, nested decorative cards, count pills, or a separate Overview and Access dashboard.

Channel membership and configuration share one flat browser inside the Channel access modal. Its Group, Project, and Channel columns are visible together, with the first available path open immediately. A fourth, continuously visible inspector configures the focused channel without hiding the hierarchy. Selecting a group or project focuses its first newly selected channel so configuration is immediately available. Clicking names navigates without changing authority. The hierarchy uses names and shared checkboxes without decorative resource icons, floating picker panels, membership cards, or separate removal controls. The main teammate editor summarizes access as a real Group, Project, and Channel tree. Group and Project names appear once, while each channel shows wrapping behavior, profile, and relevant folder information. The summary has no independent scroll area, and each channel row opens that exact channel in the modal. Closing the modal keeps the access changes in the teammate draft, and the teammate Save action still commits identity, execution, and access atomically.

A group or project checkbox selects only its current active descendant channels, never channels created later. A channel checkbox creates or removes that exact draft membership. The inspector always describes only the focused checked channel, so selecting many channels never creates a long stack of expanded configuration rows. Channel behavior uses Respond when tagged, Read and respond, and Use as context. Folder and history details appear only when the focused membership requires them. This is bulk selection of exact memberships, not inherited group authority.
