...
Membership overrides may narrow but never widen the linked profile. Reductions apply immediately and interrupt incompatible work. Every save is previewed and rechecked by the authorization service. Ordinary exact responder memberships use the teammate editor's atomic Save action without a redundant second confirmation. A concise confirmation dialog is reserved for meaningful consequences such as gaining channel-history access, gaining command or publish folder authority, or removing channel access. The dialog names the affected scope and appears over the teammate editor, never inside the channel configuration modal. Expansions create a new authorization revision and affect only new work. Active and queued work never widens automatically. Built-in profiles cannot be edited. A custom profile can be duplicated and revised, but cannot be archived while a membership still uses it.
...
Teammates is the primary complete access editor, but it keeps the same dimensions and visual grammar as every other Settings area. The resident view uses the established master-detail layout, with a compact identity directory and a larger editor for the selected teammate. Identity, execution, and channel access remain directly editable in that detail column. Channel access does not introduce a modal, wider administration surface, nested decorative cards, count pills, or a separate Overview and Access dashboard.

The main teammate editor is the access editor. It shows selected memberships as a compact Group, Project, and Channel tree, with Group and Project names appearing once. Every channel row exposes Channel behavior and Tools and folders as real compact controls instead of static summary text. A row discloses history, exact folders, targets, and approval overrides inline only when its current choices make those details relevant. At most one channel's secondary details need to be open at a time. The hierarchy has no independent scroll area, repeated ancestry strings, membership cards, or separate delete buttons.

`Add and edit access` opens an anchored cascading picker using the same interaction, width, row geometry, natural-height rules, viewport clamping, and surface grammar as the Group navigator in Projects, Notes, and Chat. The first panel lists Groups. Hovering, focusing, or activating a Group opens its Projects, and doing the same on a Project opens its active Channels. Entity icons are replaced by shared checkboxes, while Group and Project checkboxes use selected and partially selected states for their current active descendants. Unchecking is the only removal control. The main search returns compact Channel results with full Group and Project ancestry, while the Channel breakdown retains its dedicated Channel search. Each panel occupies only the height required by its visible content until viewport clamping requires scrolling. Compact layouts use the same hierarchy as a single-panel drill-down instead of forcing three panels into insufficient space. The picker changes only the unsaved teammate draft. The teammate Save action still commits identity, execution, and access atomically.

A group or project checkbox selects only its current active descendant channels, never channels created later. A channel checkbox creates or removes that exact draft membership. Selecting many channels adds compact tree rows rather than expanded configuration stacks. Channel behavior uses Respond when tagged, Read and respond, and Use as context. Folder and history details appear only when the selected membership requires them and the person discloses that row. This is bulk selection of exact memberships, not inherited group authority.

Tools and folders is one of the two primary channel controls because it communicates the effective access profile without requiring another surface. Exact folders, targets, history boundaries, and organizational runtime overrides remain secondary channel details disclosed inline only when relevant. Provider incompatibility appears only when the user selects an effective resource grant that the provider cannot enforce. Conversation-only memberships remain usable. Access-profile management is an advanced teammate utility. Persistent scratch management belongs to the reply-thread or storage lifecycle after scratch has been explicitly created, not to the primary teammate access flow.

One atomic Apply action replaces the complete editable teammate draft: display profile, role, instructions, provider policy, teammate runtime default, channel memberships, folder grants, and scratch overrides. It binds the expected teammate-profile revision, teammate-access revision, and immutable revision of every linked access profile. A concurrent teammate or profile change makes the preview or save stale instead of partially committing or silently widening authority. The studio preserves the local draft, reloads current durable state for comparison, and lets the person explicitly rebase the draft or discard it in favor of the current version. Dirty navigation is protected. Sensitive expansions and channel removals use a compact consequence-specific confirmation dialog. Validation problems remain beside the sticky Save action instead of becoming picker or hierarchy content.
