...
- `notes_page_history_settings` and `notes_page_history_snapshots`: local page recovery settings and compact snapshots of page metadata plus canonical block rows. Snapshots store the local author id in `created_by` so history and future sync operation metadata can attribute changes without adding external account data.
