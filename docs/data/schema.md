...
- `notes_pages`: page metadata, parent identity, title cache, Notion-shaped page properties, icon JSON, cover JSON, trash state, trash entry time for 7-day Trash retention, archive state, source identity, URLs, and timestamps. Page icons stay in the `icon` JSON column as validated document metadata.
