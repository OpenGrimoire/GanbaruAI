...
SQLite migrations live in `apps/client/src-tauri/migrations/` and are embedded into the desktop binary through `sqlx::migrate!("./migrations")`. The current `20260713024120_baseline_schema.sql` file is the final fresh-start schema created before the app had users. Future schema changes must add a new UTC timestamped SQL file in `YYYYMMDDHHMMSS_description.sql` form and avoid editing an already-applied migration. Do not manually register migration files; the SQLx macro discovers them at compile time. SQLx also creates `_sqlx_migrations` to track applied versions and checksums; that table is migration metadata, not app data.

The maintainer approved a final pre-user database recreation on 2026-07-12. The prior 62-file development migration history is intentionally unsupported. Close the app before resetting a development database, then recreate the selected development `ganbaru-ai.sqlite` and remove its matching `-wal` and `-shm` files if present, or create a new development Ganbaru AI folder. The app must never delete a configured vault automatically. Future schema changes use additive migrations, and the baseline becomes immutable once a user-capable release can have applied it.
...
| `name` | text | Display name. Imported calendars use the `.ics` basename, such as an email address when Google exports one calendar per email. |
...
| Field | Type | Description |
|---|---|---|
| `id` | text | Primary key. Generated by the recorder. |
| `source_type` | enum | `website`, `desktop-app`, or `mobile-app`. Mobile is reserved for later platform support. |
| `source_key` | text | Normalized host or app key used for matching current limit definitions. |
| `display_name` | text or null | Human-readable source snapshot, such as the host or app name. |
| `started_at` | integer | Unix epoch ms for the start of the measured active interval. |
| `elapsed_seconds` | integer | Positive interval duration, capped at 24 hours by schema validation. |
| `local_date` | text | User-local date (`YYYY-MM-DD`) used for daily reset at local midnight. |
| `created_at` | integer | Unix epoch ms when the sample was stored. |
...
- `notes_folders`: project-scoped navigation containers with an optional self-referencing parent folder, a trimmed display name, and timestamps. Folder parents must belong to the same project and cannot form a cycle. Deleting a folder through the command boundary promotes its direct contents before removing the row.
- `notes_pages`: page metadata, parent identity, nullable local folder placement, title cache, Notion-shaped page properties, icon JSON, cover JSON, trash state, trash entry time for 7-day Trash retention, archive state, source identity, URLs, and timestamps. `folder_id` is allowed only for workspace-parented pages and must identify a folder in the page's project. Page icons stay in the `icon` JSON column as validated document metadata.
...
- `notes_databases`, `notes_data_sources`, and `notes_database_views`: normalized child-database metadata, source property schema, and per-view presentation settings. Linked database views add another database shell and view row without duplicating the source data.
- `notes_data_source_relation_links`: normalized database relation facts derived from row page properties. Each row links one source row property to one active target row page, validates both data source sides, powers database relation backlinks and search, and can be rebuilt from `notes_pages.properties` plus `notes_data_sources.properties`.
- `notes_link_facts` and `notes_link_facts_state`: rebuildable graph facts derived from page metadata, blocks, row properties, comments, relation rows, asset references, and alias-aware local link resolution. Each fact stores typed source and target columns for pages, database rows, blocks, properties, comments, files, local app objects, and external URLs so future graph views can query typed rows instead of scanning JSON payloads live.
- `notes_data_source_rollup_cache`: rebuildable computed rollup page property values derived from relation targets. Rows are keyed by source page and rollup property, store source and target data source ids for explicit invalidation, and never replace the schema-level rollup configuration as the source of truth.
- Formula property expressions live in `notes_data_sources.properties` as schema configuration. Formula outputs are computed on database row reads from current row properties, relation titles, rollups, and other formula outputs in dependency order. They are intentionally not cached or stored in `notes_pages.properties`.
- `notes_data_source_templates` and `notes_data_source_template_blocks`: reusable database row templates scoped to one data source. The template row stores a validated property snapshot by property id, source row identity for later refresh, default-template state, and timestamps. Template blocks store canonical block snapshots separately from row pages so applying a template creates a fresh row page and fresh `notes_blocks` rows instead of pointing at the source row's body.
- Database button property configuration lives in `notes_data_sources.properties`. Button cell values are derived on database reads and are not persisted in `notes_pages.properties`. Clicking a button runs a typed local row action through a command boundary, validates the current schema and row, and writes only the target row property.
...
- `notes_page_cover_assets`: managed local image assets for Notes page covers. Rows store a content-hash id, a relative path under `assets/notes/page-covers/`, optional original file name, MIME type, byte size, SHA-256 digest, and timestamps. The table accepts PNG, JPEG, and WebP only; SVG is intentionally blocked for local cover uploads and generated covers are stored through the same table.
...
- `notes_local_users`: the privacy-preserving local Notes identity. It stores a generated local user id, display name, and timestamps only. It does not store email, avatar URL, account provider, telemetry identity, or hosted account data.
- `notes_page_history_settings` and `notes_page_history_snapshots`: the shared Notes retention setting and the active fine-grained single-page recovery format. Retention is non-null and limited to off, 7, 30, 90, 180, or 365 days. Off is stored as zero and disables new history while pruning retained versions in the affected scope.
- `notes_history_bundles` and `notes_history_bundle_chunks`: immutable content-addressed storage for canonical project history rows and manifests. `hash` is the SHA-256 digest of canonical uncompressed bytes. Because identical bytes share one hash, `kind` records the first role that stored the bytes and is not a role-validation discriminator. Encoding records raw, zlib, or bounded chunked storage. Declared and stored sizes are constrained and verified before JSON parsing.
- `notes_project_history_versions`: ordered project checkpoints with manifest hash, actor and display-name snapshot, reason, changed-note summary, page state counts, and timestamp. The project foreign key removes versions when the owning project is deleted.
- `notes_project_history_bundle_references`: many-to-many pins from retained versions to deduplicated row bundles. `notes_project_history_asset_pins` performs the same role for managed local assets. `notes_project_history_dirty` stores the first and latest edit times, current actor snapshot, summary, and forced-checkpoint state so checkpoint cadence survives restarts. Its deadline index supports the app-level scheduler without scanning history payloads. `notes_history_maintenance_state` stores the last bounded retention and orphan-maintenance pass for the active vault, so tab mounts and ordinary checkpoint checks do not repeat global cleanup.
- `notes_undo_state`: bounded page-local undo and redo state derived from canonical page and block rows.
- `notes_comment_threads`, `notes_comments`, and `notes_comment_thread_anchors`: canonical Notes discussions, comment rows, and optional inline rich text anchors. Threads target pages or blocks, comments store validated rich text plus the local user id and a display-name snapshot, and anchors store selected block text plus offsets and short context for practical relocation after nearby edits.

## Project tables

Project management stores structured work data in SQLite. The user-facing hierarchy is group, project, section, task.

- `project_groups`: top-level containers such as a company name or Routine. The `icon` column stores a canonical prefixed icon source value. `group-routine` is the protected built-in group identity. Its stored canonical name is not the localized UI label.
- `projects`: scheduleable units under a group. Calendar events can reference a project through `calendar_events.project_id`. The `project-routine-*` ids identify protected built-in Routine projects whose localized names are derived in the client. One read-only integrity query precedes the initial Projects workspace read. Only a failed integrity result starts the set-based repair transaction that restores missing built-in rows and normalizes their stored canonical name, Routine membership, and sort order. Ordinary project refreshes never run repair. Existing icon, color, lifecycle, scheduling, and other allowed settings are preserved. Projects store an optional default event duration, event color, explicit Default Pomodoro mode, optional preset key, optional custom count rhythm fields, idle-pause minutes, focus playlist id, break playlist id, work environment id, blocker ruleset id, an `icon` value, the nullable Notes default open mode override, and the nullable `notes_history_retention_days` override. A null Notes history value inherits the shared Notes setting. Non-null values are limited to off, 7, 30, 90, 180, or 365 days, with off stored as zero. A missing default event duration uses the app default when creating scheduled blocks. A null default event name on a built-in Routine project derives the current localized project name, while a non-null value is an explicit user override. Default Pomodoro mode is `none`, `preset`, or `custom`; `none` creates project events without Pomodoro, `preset` copies the selected preset, and `custom` copies the stored focus, short break, long break, and cycle count values. Focus playlist and work environment defaults can be copied into new calendar events now. Break playlist and blocker ruleset defaults are stored for the future runtime surfaces that will consume them.
- `project_working_folders`: project-owned coding and Markdown roots. Each row has stable identity, non-null project ownership, display name, managed or external kind, optional managed vault-relative path, repository probe identity, sort order, archive state, revision, and timestamps. A partial unique index and managed-row protection triggers enforce exactly one non-archivable managed row per project. `(id, project_id)` is unique so Chat can prove folder ownership with a composite foreign key. External absolute paths and verification state are not stored here.
...
Every fresh or repaired built-in project also receives its managed working-folder row. Project creation inserts the project and its managed row in one transaction after creating `projects/{project-id}/`. The obsolete `chat_workspaces` table is absent. Current `chat_threads` rows represent provider continuations and execution timelines. Their `project_id` and `execution_environment_id` are non-null. A folder-backed thread also carries its matching `working_folder_id`; a scratch-backed thread instead carries an execution environment linked to its exact scratch generation. Foreign keys and target-consistency triggers require the chosen environment to belong to the same project and prevent a thread from mixing folder and scratch identity. Folder-owned drafts, provider-session attachments, checkpoints, cleanup rows, terminals, mentions, provider sessions, and filesystem tools retain their exact working-folder identity. Scratch execution shells remain reply-thread scoped and do not enter ordinary folder-thread navigation. Channel-owned scratch attachments keep channel authorization in the promotion record. Their derived managed-folder field is only a compatibility association for the existing attachment index.

## Chat coordination

The coordination schema is the canonical organizational and authorization layer above the existing provider execution tables. [Chat access control](access-control.md) defines the normative meaning of these records.
...
- Generic conversation membership is separate from AI-specific access. AI channel access stores independent read-history and participate capabilities, an entire-history or captured from-grant boundary, linked access-profile revision, runtime override, optimistic access revision, removal, and revocation state.
- Stable access profiles point to immutable revisions that store default channel capabilities, default history boundary, and maximum folder capability. Built-in profile identities are immutable and duplicable. A membership may narrow but never widen its linked profile.
- Folder grants link one channel membership to exact folders in that channel's project. Each stores capability, optional runtime override, optional default-target state, logical repository identity, revision, and revocation. Zero or one default is valid. Grants from different channels are never combined.
- `chat_reply_threads`, `chat_conversation_items`, `chat_communication_messages`, and immutable message revisions own channel roots, replies, meaningful work entries, ordered paging, attachments, read cursors, and destination-scoped drafts. Common immutable reference rows plus typed extensions represent participant, channel, working-folder, workspace-path, and execution-environment references without embedding authority.
- Channel-reference authorization records freeze source and destination, message and audience revisions, source history bounds, source high-water marks, revision cutoffs, authorization revision, and scope digest. Host-tool audits record returned immutable message revisions and hashes without duplicating bodies.
- Assignment authorization revisions store the exact channel and folder sources admitted to a run. Continuation rows store authorization-scope digests. Durable revocation and cleanup jobs stop or repair work that loses authority.
- Logical scratch scopes and generations are keyed by vault, reply thread, and teammate. SQLite stores lifecycle, bounded size estimates, provenance, and retained source constraints, never the scratch absolute path or current device availability. Execution environments represent current folders, existing worktrees, and scratch generations. Scratch promotion rows retain revision-checked logical sources, idempotency digests, and exact destination channels. A project-folder promotion records its authorized folder, while a channel-owned managed attachment records no folder authority. The attachment table's internally derived managed-folder association remains a storage and indexing detail only. Cleanup-job rows retain explicit confirmation, optimistic scope revision, retry state, and removed-byte outcomes without storing a filesystem path.
...
- **Teammate policies and channel grants:** provider instance, model, effort, speed, provider options, teammate runtime default, channel capabilities, history boundary, profile ceiling, exact folder grants, and optional default target.
- **Conversation memberships:** participant role, revision, removal state, and separate AI-specific access. Membership alone grants no history, filesystem, or provider capability.
- **Communication entries:** human and materialized AI-teammate messages, thread roots and replies, semantic work updates, immutable revisions, typed references, attachments, stable ordering, FTS indexing, and idempotent command receipts.
...
- **Agent runs:** bounded response or execution attempts permanently linked to one project, one immutable teammate policy revision, one revisioned authorization decision, one provider turn, and optionally one hidden provider thread. Conversation runs have no native target. File or command runs bind one folder or explicit private-scratch target.
- **Future memory entries and provenance:** bounded teammate memories remain deferred. When added, they require source revisions, confidence, scope, retention, invalidation state, and the same disclosure checks as direct channel history.
...
- **Future access surfaces:** group and Notes scopes, budgets, invitations, human history visibility, durable teammate memory, subscriptions, and broader derived-read auditing remain later work.
...
Access enforcement applies before direct Chat reads, references, provider dispatch, host-tool calls, continuation reuse, scratch reuse, and result publication. Future search, backlinks, dashboards, notifications, reports, exports, summaries, and other derived surfaces must use the same service and cannot reveal inaccessible identities or aggregates.
...
- **Future Notes indexes:** new Notes projections for tags, anchors, or ranking must remain rebuildable from canonical Notes rows and managed asset references, not markdown exports.
