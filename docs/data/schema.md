...
The maintainer approved a clean local Ganbaru AI folder recreation on 2026-07-10 for the current Projects default identity and icon-color work. This pre-user batch updates the baseline and its default-project migrations directly. Later builds must return to additive migrations once these files can have been applied outside this reset.
...
- `notes_page_history_settings` and `notes_page_history_snapshots`: the shared Notes retention setting and the compatible single-page recovery format. Retention is non-null and limited to off, 7, 30, 90, 180, or 365 days. Off is stored as zero and disables new history while pruning retained versions in the affected scope. Legacy null or values above 365 map to 365, while other unsupported finite values round up to the next supported period.
...
- `project_groups`: top-level containers such as a company name or Routine. The `icon` column stores a legacy Lucide slug or a prefixed icon source value. `group-routine` is the protected built-in group identity. Its stored canonical name is not the localized UI label.
