...
- **Chat storage boundary:** project working folders, channels, ordered channel-session links, provider threads, canonical events, projections, drafts, attachment metadata, command receipts, checkpoints, and cleanup records live in the active vault SQLite database. Every provider thread has a non-null project and working folder. Personal channel sections and last-selected-channel state are device-local presentation preferences. Managed attachment bytes live under the active vault at `assets/chat/attachments/`. External absolute folder paths, executable paths, provider homes, probe caches, diagnostics, full-access trust, and process state are device-local. Provider secrets live only behind operating-system credential references.
